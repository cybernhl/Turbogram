...
public class ViewCompat {
    ...
    static class UnhandledKeyEventManager {
        ...
        static UnhandledKeyEventManager at(View root) {
            UnhandledKeyEventManager manager = (UnhandledKeyEventManager) root.getTag(C0028R.id.tag_unhandled_key_event_manager);
            if (manager != null) {
                return manager;
            }
            manager = new UnhandledKeyEventManager();
            root.setTag(C0028R.id.tag_unhandled_key_event_manager, manager);
            return manager;
        }
        ...
        boolean preDispatch(KeyEvent event) {
            ...
            if (event.getAction() == 1) {
                ...
                if (idx >= 0) {
                    weakReference = (WeakReference) capturedKeys.valueAt(idx);
                    capturedKeys.removeAt(idx);
                }
            }
            if (weakReference == null) {
                weakReference = (WeakReference) capturedKeys.get(event.getKeyCode());
            }
            if (weakReference == null) {
                return false;
            }
        ...
        private boolean onUnhandledKeyEvent(@NonNull View v, @NonNull KeyEvent event) {
            ArrayList<OnUnhandledKeyEventListenerCompat> viewListeners = (ArrayList) v.getTag(C0028R.id.tag_unhandled_key_listeners);
    ...
    public static boolean startDragAndDrop(@NonNull View v, ClipData data, DragShadowBuilder shadowBuilder, Object localState, int flags) {
        if (VERSION.SDK_INT >= 24) {
    ...
    public static void updateDragShadow(@NonNull View v, DragShadowBuilder shadowBuilder) {
        if (VERSION.SDK_INT >= 24) {
    ...
    public static void addOnUnhandledKeyEventListener(@NonNull View v, @NonNull OnUnhandledKeyEventListenerCompat listener) {
        if (VERSION.SDK_INT >= 28) {
            Map<OnUnhandledKeyEventListenerCompat, OnUnhandledKeyEventListener> viewListeners = (Map) v.getTag(C0028R.id.tag_unhandled_key_listeners);
            if (viewListeners == null) {
                viewListeners = new ArrayMap();
                v.setTag(C0028R.id.tag_unhandled_key_listeners, viewListeners);
            }
            OnUnhandledKeyEventListener fwListener = new OnUnhandledKeyEventListenerWrapper(listener);
            viewListeners.put(listener, fwListener);
            v.addOnUnhandledKeyEventListener(fwListener);
            return;
        }
        ArrayList<OnUnhandledKeyEventListenerCompat> viewListeners2 = (ArrayList) v.getTag(C0028R.id.tag_unhandled_key_listeners);
        if (viewListeners2 == null) {
            viewListeners2 = new ArrayList();
            v.setTag(C0028R.id.tag_unhandled_key_listeners, viewListeners2);
        }

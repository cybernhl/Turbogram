...
public class FileUploadOperation {
    ...
    class C08006 implements WriteToSocketDelegate {
        ...
        public void run() {
            Utilities.stageQueue.postRunnable(new C07991());
        }
    }
    ...
    public void start() {
        if (this.state == 0) {
            this.state = 1;
            Utilities.stageQueue.postRunnable(new C07941());
        }
    }
    ...
    protected void onNetworkChanged(final boolean slow) {
        if (this.state == 1) {
            Utilities.stageQueue.postRunnable(new Runnable() {
                public void run() {
                    int count = 1;
                    if (FileUploadOperation.this.slowNetwork != slow) {
                        int a;
                        ...
                        FileUploadOperation.this.operationGuid = FileUploadOperation.this.operationGuid + 1;
                        a = 0;
    ...
    public void cancel() {
        if (this.state != 3) {
            this.state = 2;
            Utilities.stageQueue.postRunnable(new C07963());
        ...
        if (this.state == 1) {
            try {
                ...
                if (this.stream == null) {
                    ...
                    this.uploadChunkSize = (int) Math.max(this.slowNetwork ? 32 : 128, ((this.totalFileSize + 3072000) - 1) / 3072000);
                ...
                if (this.estimatedSize == 0 || this.readBytesCount + ((long) this.uploadChunkSize) <= this.availableSize) {
                    ...
                    if (currentRequestBytes != -1) {
                        ...
                        if (this.slowNetwork) {
                            ...
                            connectionType = ((requestNumFinal % 4) << 16) | 4;
                        }
                        ...
                        this.requestTokens.put(requestNumFinal, ConnectionsManager.getInstance(this.currentAccount).sendRequest(finalRequest, new RequestDelegate() {
                            public void run(TLObject response, TLRPC$TL_error error) {
                                if (currentOperationGuid == FileUploadOperation.this.operationGuid) {
                        ...
                        }, null, new C08006(), 0, Integer.MAX_VALUE, connectionType, true));
                    }
                }
            } catch (Exception e2) {
                FileLog.e(e2);

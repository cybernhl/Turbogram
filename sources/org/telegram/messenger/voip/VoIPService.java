...
public class VoIPService extends VoIPBaseService {
    ...
    class C09483 implements RequestDelegate {
        C09483() {
        }
    ...
    class C09587 implements Runnable {
        C09587() {
        }
            ...
            if (this.user == null) {
                ...
                if (this.isOutgoing) {
                    ...
                    if (USE_CONNECTION_SERVICE) {
                        ...
                        this.delayedStartOutgoingCall = new C09461();
    ...
    protected void onControllerPreRelease() {
        if (this.needSendDebugLog) {
            ...
            ConnectionsManager.getInstance(this.currentAccount).sendRequest(req, new C09483());
        }
    }
    ...
    private void startOutgoingCall() {
        ...
        dispatchStateChanged(14);
        AndroidUtilities.runOnUIThread(new C09494());
                ...
                if (error == null) {
                    ...
                    if (response instanceof TLRPC$TL_messages_dhConfig) {
                        if (Utilities.isGoodPrime(res.f804p, res.f803g)) {
                            messagesStorage.setSecretPBytes(res.f804p);
                            messagesStorage.setSecretG(res.f803g);
                        ...
                        public void run(final TLObject response, final TLRPC$TL_error error) {
                            AndroidUtilities.runOnUIThread(new Runnable() {
                                ...
                                class C09521 implements Runnable {
                                    ...
                                    class C09511 implements RequestDelegate {
                                        ...
                                        public void run(TLObject response, TLRPC$TL_error error) {
                                            ...
                                            AndroidUtilities.runOnUIThread(new C09501());
                                        }
                                    }
                                    ...
                                    public void run() {
                                        ...
                                        ConnectionsManager.getInstance(VoIPService.this.currentAccount).sendRequest(req, new C09511(), 2);
                                    }
                                }
                                ...
                                public void run() {
                                    if (error == null) {
                                        ...
                                        VoIPService.this.timeoutRunnable = new C09521();
    ...
    public void acceptIncomingCall() {
        ...
        AndroidUtilities.runOnUIThread(new C09587());
        final MessagesStorage messagesStorage = MessagesStorage.getInstance(this.currentAccount);
        TLRPC$TL_messages_getDhConfig req = new TLRPC$TL_messages_getDhConfig();
        req.random_length = 256;
        req.version = messagesStorage.getLastSecretVersion();
        ConnectionsManager.getInstance(this.currentAccount).sendRequest(req, new RequestDelegate() {
            ...
            class C09601 implements RequestDelegate {
                C09601() {
                }
            ...
            public void run(TLObject response, TLRPC$TL_error error) {
                if (error == null) {
                    ...
                    ConnectionsManager.getInstance(VoIPService.this.currentAccount).sendRequest(req, new C09601(), 2);
                    return;
                }
                VoIPService.this.callFailed();
            }

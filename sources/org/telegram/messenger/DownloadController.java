...
public class DownloadController implements NotificationCenter$NotificationCenterDelegate {
    ...
    public DownloadController(int instance) {
        ...
        AndroidUtilities.runOnUIThread(new DownloadController$1(this));
        ApplicationLoader.applicationContext.registerReceiver(new DownloadController$2(this), new IntentFilter("android.net.conn.CONNECTIVITY_CHANGE"));
        ...
        if (currentMask != this.lastCheckMask) {
            ...
            if ((currentMask & 1) == 0) {
                for (a = 0; a < this.photoDownloadQueue.size(); a++) {
                    FileLoader.getInstance(this.currentAccount).cancelLoadFile((TLRPC$PhotoSize) ((DownloadObject) this.photoDownloadQueue.get(a)).object);
                }
                this.photoDownloadQueue.clear();
            ...
            if ((currentMask & 2) == 0) {
                for (a = 0; a < this.audioDownloadQueue.size(); a++) {
                    FileLoader.getInstance(this.currentAccount).cancelLoadFile((TLRPC$Document) ((DownloadObject) this.audioDownloadQueue.get(a)).object);
                }
                this.audioDownloadQueue.clear();
            ...
            if ((currentMask & 64) == 0) {
                for (a = 0; a < this.videoMessageDownloadQueue.size(); a++) {
                    FileLoader.getInstance(this.currentAccount).cancelLoadFile((TLRPC$Document) ((DownloadObject) this.videoMessageDownloadQueue.get(a)).object);
                }
                this.videoMessageDownloadQueue.clear();
            ...
            if ((currentMask & 4) == 0) {
                for (a = 0; a < this.videoDownloadQueue.size(); a++) {
                    FileLoader.getInstance(this.currentAccount).cancelLoadFile((TLRPC$Document) ((DownloadObject) this.videoDownloadQueue.get(a)).object);
                }
                this.videoDownloadQueue.clear();
            ...
            if ((currentMask & 16) == 0) {
                for (a = 0; a < this.musicDownloadQueue.size(); a++) {
                    FileLoader.getInstance(this.currentAccount).cancelLoadFile((TLRPC$Document) ((DownloadObject) this.musicDownloadQueue.get(a)).object);
                }
                this.musicDownloadQueue.clear();
            ...
            if ((currentMask & 32) == 0) {
                for (a = 0; a < this.gifDownloadQueue.size(); a++) {
                    FileLoader.getInstance(this.currentAccount).cancelLoadFile((TLRPC$Document) ((DownloadObject) this.gifDownloadQueue.get(a)).object);
                }
                this.gifDownloadQueue.clear();
    ...
    public boolean canDownloadMedia(TLRPC$Message message) {
        if (TurboConfig$Tabs.autoDownloadFavorite && TurboConfig$Tabs.selectedTab == 5) {
            return true;
        }
        if (!this.globalAutodownloadEnabled) {
            return false;
            ...
            for (int a = 0; a < objects.size(); a++) {
                ...
                if (!this.downloadQueueKeys.containsKey(path)) {
                    ...
                    if (downloadObject.object instanceof TLRPC$PhotoSize) {
                        FileLoader.getInstance(this.currentAccount).loadFile((TLRPC$PhotoSize) downloadObject.object, null, downloadObject.secret ? 2 : 0);
    ...
    public void addLoadingFileObserver(String fileName, DownloadController$FileDownloadProgressListener observer) {
        addLoadingFileObserver(fileName, null, observer);
    }

    public void addLoadingFileObserver(String fileName, MessageObject messageObject, DownloadController$FileDownloadProgressListener observer) {
        if (this.listenerInProgress) {
            this.addLaterArray.put(fileName, observer);
            return;
        }
        removeLoadingFileObserver(observer);
        ArrayList<WeakReference<DownloadController$FileDownloadProgressListener>> arrayList = (ArrayList) this.loadingFileObservers.get(fileName);
        if (arrayList == null) {
    ...
    public void removeLoadingFileObserver(DownloadController$FileDownloadProgressListener observer) {
        if (this.listenerInProgress) {
        ...
        if (fileName != null) {
            ...
            if (arrayList != null) {
                ...
                while (a < arrayList.size()) {
                    WeakReference<DownloadController$FileDownloadProgressListener> reference = (WeakReference) arrayList.get(a);
    ...
    private void processLaterArrays() {
        for (Entry<String, DownloadController$FileDownloadProgressListener> listener : this.addLaterArray.entrySet()) {
            addLoadingFileObserver((String) listener.getKey(), (DownloadController$FileDownloadProgressListener) listener.getValue());
        }
        this.addLaterArray.clear();
        Iterator it = this.deleteLaterArray.iterator();
        while (it.hasNext()) {
            removeLoadingFileObserver((DownloadController$FileDownloadProgressListener) it.next());
        }
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        String fileName;
        ArrayList<WeakReference<DownloadController$FileDownloadProgressListener>> arrayList;
        int size;
        int a;
        WeakReference<DownloadController$FileDownloadProgressListener> reference;
        if (id == NotificationCenter.FileDidFailedLoad || id == NotificationCenter.httpFileDidFailedLoad) {
            ...
            if (arrayList != null) {
                ...
                for (a = 0; a < size; a++) {
                    ...
                    if (reference.get() != null) {
                        ((DownloadController$FileDownloadProgressListener) reference.get()).onFailedDownload(fileName);
                        this.observersByTag.remove(((DownloadController$FileDownloadProgressListener) reference.get()).getObserverTag());
                    }
                }
                this.loadingFileObservers.remove(fileName);
            }
            ...
            if (arrayList != null) {
                ...
                for (a = 0; a < size; a++) {
                    reference = (WeakReference) arrayList.get(a);
                    if (reference.get() != null) {
                        ((DownloadController$FileDownloadProgressListener) reference.get()).onSuccessDownload(fileName);
                        this.observersByTag.remove(((DownloadController$FileDownloadProgressListener) reference.get()).getObserverTag());
                    }
                }
                this.loadingFileObservers.remove(fileName);
            ...
            if (arrayList != null) {
                ...
                for (a = 0; a < size; a++) {
                    reference = (WeakReference) arrayList.get(a);
                    if (reference.get() != null) {
                        ((DownloadController$FileDownloadProgressListener) reference.get()).onProgressDownload(fileName, progress.floatValue());
                    }
                }
            }
            this.listenerInProgress = false;
            ...
            if (arrayList != null) {
                ...
                for (a = 0; a < size; a++) {
                    reference = (WeakReference) arrayList.get(a);
                    if (reference.get() != null) {
                        ((DownloadController$FileDownloadProgressListener) reference.get()).onProgressUpload(fileName, progress.floatValue(), enc.booleanValue());
                    }
                }
            }
            this.listenerInProgress = false;
            processLaterArrays();
            try {
                ArrayList<SendMessagesHelper$DelayedMessage> delayedMessages = SendMessagesHelper.getInstance(this.currentAccount).getDelayedMessages(fileName);

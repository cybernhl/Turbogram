...
class MediaController$23 implements Runnable {
    ...
    class C08451 implements Comparator<MediaController$PhotoEntry> {
        C08451() {
        }
    ...
    public void run() {
        ...
        SparseArray<MediaController$AlbumEntry> mediaAlbums = new SparseArray();
        SparseArray<MediaController$AlbumEntry> photoAlbums = new SparseArray();
        ...
        if (VERSION.SDK_INT < 23 || (VERSION.SDK_INT >= 23 && ApplicationLoader.applicationContext.checkSelfPermission("android.permission.READ_EXTERNAL_STORAGE") == 0)) {
            cursor = Media.query(ApplicationLoader.applicationContext.getContentResolver(), Media.EXTERNAL_CONTENT_URI, MediaController.access$6400(), null, null, "datetaken DESC");
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    try {
                        ...
                        if (!(path == null || path.length() == 0)) {
                            ...
                            albumEntry = (MediaController$AlbumEntry) mediaAlbums.get(bucketId);
                            if (albumEntry == null) {
                                mediaController$AlbumEntry = new MediaController$AlbumEntry(bucketId, bucketName, photoEntry);
                                mediaAlbums.put(bucketId, mediaController$AlbumEntry);
                            ...
                            albumEntry = (MediaController$AlbumEntry) photoAlbums.get(bucketId);
                            if (albumEntry == null) {
                                mediaController$AlbumEntry = new MediaController$AlbumEntry(bucketId, bucketName, photoEntry);
                                photoAlbums.put(bucketId, mediaController$AlbumEntry);
        ...
        if (cursor != null) {
            ...
            if (VERSION.SDK_INT < 23 || (VERSION.SDK_INT >= 23 && ApplicationLoader.applicationContext.checkSelfPermission("android.permission.READ_EXTERNAL_STORAGE") == 0)) {
                cursor = Media.query(ApplicationLoader.applicationContext.getContentResolver(), Video.Media.EXTERNAL_CONTENT_URI, MediaController.access$6500(), null, null, "datetaken DESC");
                if (cursor != null) {
                    imageIdColumn = cursor.getColumnIndex("_id");
                    bucketIdColumn = cursor.getColumnIndex("bucket_id");
                    bucketNameColumn = cursor.getColumnIndex("bucket_display_name");
            ...
            for (a = 0; a < mediaAlbumsSorted.size(); a++) {
                Collections.sort(((MediaController$AlbumEntry) mediaAlbumsSorted.get(a)).photos, new C08451());
            }
            MediaController.access$6600(this.val$guid, mediaAlbumsSorted, photoAlbumsSorted, num, mediaController$AlbumEntry3, mediaController$AlbumEntry2, 0);
        }
        mediaCameraAlbumId = num;
        allMediaAlbum = mediaController$AlbumEntry3;
        try {

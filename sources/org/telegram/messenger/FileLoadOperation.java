...
public class FileLoadOperation {
    ...
    class C07764 implements Runnable {
        C07764() {
        }
    ...
    class C07797 implements Runnable {
        C07797() {
        }
    ...
    class C07808 implements Runnable {
        C07808() {
        }
        ...
        try {
            ...
            if (this.ext != null) {
                ...
                if (idx != -1) {
                    ...
                    if (this.ext.length() > 1 && TurboConfig$Storage.keepFilename && !this.ext.contains("webp") && !this.ext.contains(".mp4") && !this.ext.contains(".gif") && !this.ext.contains(".ogg")) {
    ...
    public void pause() {
        if (this.state == 1) {
            Utilities.stageQueue.postRunnable(new C07764());
        }
    }
    ...
    public boolean start(FileStreamLoadOperation stream, int streamOffset) {
        ...
        if (stream != null) {
            ...
            Utilities.stageQueue.postRunnable(new C07786());
        }
        if (alreadyStarted) {
            return wasPaused;
        }
        ...
        this.state = 1;
        if (TurboConfig$Storage.keepFilename && orgName != null) {
        ...
        if (!(!finalFileExist || this.totalBytesCount == 0 || ((long) this.totalBytesCount) == this.cacheFileFinal.length() || orgName == null || !TurboConfig$Storage.keepFilename)) {
        ...
        if (finalFileExist) {
            ...
            this.started = true;
            Utilities.stageQueue.postRunnable(new C07797());
        }
        return true;
    }
    ...
    public void cancel() {
        Utilities.stageQueue.postRunnable(new C07808());
    }
    ...
    private void onFinishLoadingFile(final boolean increment) {
        if (this.state == 1) {
            ...
            if (!(this.cacheFileTemp == null || TurboUtils.renameTo(this.cacheFileTemp, this.cacheFileFinal))) {
                if (BuildVars.LOGS_ENABLED) {
        ...
        if (error == null) {
            try {
                if (this.notLoadedBytesRanges != null || this.downloadedBytes == requestInfo.offset) {
                    ...
                    if (this.isCdn) {
                        ...
                        if ((this.cdnHashes != null ? (TLRPC$TL_fileHash) this.cdnHashes.get(fileOffset) : null) == null) {
                    ...
                    if (this.isCdn) {
                        ...
                        if (!checked) {
                            ...
                            if (availableSize != 0 && (availableSize == 131072 || ((this.totalBytesCount > 0 && availableSize == this.totalBytesCount - fileOffset) || (this.totalBytesCount <= 0 && finishedDownloading)))) {
                                TLRPC$TL_fileHash hash = (TLRPC$TL_fileHash) this.cdnHashes.get(fileOffset);
                                if (this.fileReadStream == null) {
            ...
            if (BuildVars.LOGS_ENABLED) {
                if (this.location != null) {
                    ...
                    FileLog.e("" + this.webLocation + " id = " + getFileName());
                }
            }
            onFail(false, 0);
        }
            ...
            while (a < count) {
                ...
                if (this.totalBytesCount <= 0 || downloadOffset < this.totalBytesCount) {
                    ...
                    RequestDelegate anonymousClass12 = new RequestDelegate() {
                        ...
                        class C07721 implements RequestDelegate {
                            C07721() {
                            }
                            ...
                            if (error != null && (request instanceof TLRPC$TL_upload_getCdnFile) && error.text.equals("FILE_TOKEN_INVALID")) {
                                ...
                                if (!res.file_hashes.isEmpty()) {
                                    ...
                                    for (int a = 0; a < res.file_hashes.size(); a++) {
                                        TLRPC$TL_fileHash hash = (TLRPC$TL_fileHash) res.file_hashes.get(a);
                                        FileLoadOperation.this.cdnHashes.put(hash.offset, hash);
                                    }
                                }
                                ...
                                if (response instanceof TLRPC$TL_upload_file) {
                                    requestInfo2.response = (TLRPC$TL_upload_file) response;
                                } else if (response instanceof TLRPC$TL_upload_webFile) {
                                    requestInfo2.responseWeb = (TLRPC$TL_upload_webFile) response;
                                    ...
                                    requestInfo2.responseCdn = (TLRPC$TL_upload_cdnFile) response;
                                }
                                if (response != null) {
                                ...
                                ConnectionsManager.getInstance(FileLoadOperation.this.currentAccount).sendRequest(req, new C07721(), null, null, 0, FileLoadOperation.this.datacenterId, 1, true);
                            }
                        }
                    };
                    if (this.isCdn) {

...
public class SharedConfig {
    ...
    public static ArrayList<SharedConfig$ProxyInfo> proxyList = new ArrayList();
        ...
        if (!proxyListLoaded) {
            ...
            if (!TextUtils.isEmpty(list)) {
                ...
                for (int a = 0; a < count; a++) {
                    info = new SharedConfig$ProxyInfo(data.readString(false), data.readInt32(false), data.readString(false), data.readString(false), data.readString(false));
                    proxyList.add(info);
            ...
            if (currentProxy == null && !TextUtils.isEmpty(proxyAddress)) {
                info = new SharedConfig$ProxyInfo(proxyAddress, proxyPort, proxyUsername, proxyPassword, proxySecret);
                currentProxy = info;
    ...
    public static void saveProxyList() {
        ...
        for (int a = 0; a < count; a++) {
            String str;
            SharedConfig$ProxyInfo info = (SharedConfig$ProxyInfo) proxyList.get(a);
    ...
    public static SharedConfig$ProxyInfo addProxy(SharedConfig$ProxyInfo proxyInfo) {
        loadProxyList();
        int count = proxyList.size();
        for (int a = 0; a < count; a++) {
            SharedConfig$ProxyInfo info = (SharedConfig$ProxyInfo) proxyList.get(a);
    ...
    public static void deleteProxy(SharedConfig$ProxyInfo proxyInfo) {

...
public class LocaleController {
    ...
    public LocaleController() {
        int a;
        addRules(new String[]{"bem", "brx", "da", "de", "el", "en", "eo", "es", "et", "fi", "fo", "gl", "he", "iw", "it", "nb", "nl", "nn", "no", "sv", "af", "bg", "bn", "ca", "eu", "fur", "fy", "gu", "ha", "is", "ku", "lb", "ml", "mr", "nah", "ne", "om", "or", "pa", "pap", "ps", "so", "sq", "sw", "ta", "te", "tk", "ur", "zu", "mn", "gsw", "chr", "rm", "pt", "an", "ast"}, new LocaleController$PluralRules_One());
        ...
        addRules(new String[]{"az", "bm", "fa", "ig", "hu", "ja", "kde", "kea", "ko", "my", "ses", "sg", "to", "tr", "vi", "wo", "yo", "zh", "bo", "dz", TtmlNode.ATTR_ID, "jv", "jw", "ka", "km", "kn", "ms", "th", "in"}, new LocaleController$PluralRules_None());
        ...
        if (this.remoteLanguages.isEmpty()) {
            AndroidUtilities.runOnUIThread(new LocaleController$$Lambda$0(this));
        }
    ...
    final /* synthetic */ void lambda$new$0$LocaleController() {
    ...
    private LocaleController$LocaleInfo getLanguageFromDict(String key) {
        ...
        return (LocaleController$LocaleInfo) this.languagesDict.get(key.toLowerCase().replace("-", "_"));
    }
    ...
    public boolean applyLanguageFile(File file, int currentAccount) {
        try {
            ...
            if (languageName != null && languageName.length() > 0 && languageNameInEnglish != null && languageNameInEnglish.length() > 0 && languageCode != null && languageCode.length() > 0) {
                ...
                LocaleController$LocaleInfo localeInfo = getLanguageFromDict("local_" + languageCode.toLowerCase());
                if (localeInfo == null) {
                    localeInfo = new LocaleController$LocaleInfo();
    ...
    public boolean deleteLanguage(LocaleController$LocaleInfo localeInfo, int currentAccount) {
    ...
    public void applyLanguage(LocaleController$LocaleInfo localeInfo, boolean override, boolean init, int currentAccount) {
        applyLanguage(localeInfo, override, init, false, false, currentAccount);
    }

    public void applyLanguage(LocaleController$LocaleInfo localeInfo, boolean override, boolean init, boolean fromFile, boolean force, int currentAccount) {
        if (localeInfo != null) {
            ...
            if (localeInfo.isRemote() && (force || !pathToFile.exists())) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1221d("reload locale because file doesn't exist " + pathToFile);
                }
                if (init) {
                    AndroidUtilities.runOnUIThread(new LocaleController$$Lambda$1(this, localeInfo, currentAccount));
                } else {
            ...
            try {
                ...
                if (this.reloadLastFile) {
                    if (init) {
                        AndroidUtilities.runOnUIThread(new LocaleController$$Lambda$2(this, currentAccount));
                    } else {
    ...
    final /* synthetic */ void lambda$applyLanguage$1$LocaleController(LocaleController$LocaleInfo localeInfo, int currentAccount) {
        applyRemoteLanguage(localeInfo, true, currentAccount);
    }

    final /* synthetic */ void lambda$applyLanguage$2$LocaleController(int currentAccount) {
        reloadCurrentRemoteLocale(currentAccount);
    }
        ...
        try {
            ...
            if (TurboConfig.persianDate) {
                ...
                if (year == rightNow.get(1)) {
                    return new SolarCalendar(rightNow).getShortDesDate();
                }
                return new SolarCalendar(rightNow).getNumDate();
    ...
    public static String formatDateOnline(long date) {
        ...
        try {
            ...
            if (dateDay == day && year == dateYear) {
                if (TurboConfig.persianDate) {
                    return getString("LastSeen", R.string.LastSeen) + " " + getString("TodayAt", R.string.TodayAt) + " " + new SolarCalendar(rightNow).getTime();
                }
                objArr = new Object[1];
                objArr[0] = formatString("TodayAtFormatted", R.string.TodayAtFormatted, getInstance().formatterDay.format(new Date(date)));
                ...
                if (TurboConfig.persianDate) {
                    return getString("LastSeen", R.string.LastSeen) + " " + getString("YesterdayAt", R.string.YesterdayAt) + " " + new SolarCalendar(rightNow).getTime();
                }
                objArr = new Object[1];
                objArr[0] = formatString("YesterdayAtFormatted", R.string.YesterdayAtFormatted, getInstance().formatterDay.format(new Date(date)));
                return formatString("LastSeenFormatted", R.string.LastSeenFormatted, objArr);
                ...
                if (TurboConfig.persianDate) {
                    return getString("LastSeen", R.string.LastSeen) + " " + new SolarCalendar(rightNow).getShortDesDateTime();
                }
                format = formatString("formatDateAtTime", R.string.formatDateAtTime, getInstance().formatterMonth.format(new Date(date)), getInstance().formatterDay.format(new Date(date)));
                return formatString("LastSeenDateFormatted", R.string.LastSeenDateFormatted, format);
            } else if (TurboConfig.persianDate) {
                return getString("LastSeen", R.string.LastSeen) + " " + new SolarCalendar(rightNow).getNumDateTime();
            } else {
                format = formatString("formatDateAtTime", R.string.formatDateAtTime, getInstance().formatterYear.format(new Date(date)), getInstance().formatterDay.format(new Date(date)));
                return formatString("LastSeenDateFormatted", R.string.LastSeenDateFormatted, format);
            }
    ...
    public void recreateFormatters() {
        ...
        boolean z2 = lang.startsWith("ar") || lang.startsWith("fa") || lang.startsWith("he") || lang.startsWith("iw");
        isRTL = z2;
        if (lang.equals("ko")) {
            i = 2;
        } else {
        ...
        nameDisplayOrder = i;
        isPersian = lang.toLowerCase().equals("fa");
        if (TurboConfig.flipDirection) {
            if (isRTL) {
                z = false;
            }
            isRTL = z;
    ...
    public void saveRemoteLocaleStrings(TLRPC$TL_langPackDifference difference, int currentAccount) {
        if (difference != null && !difference.strings.isEmpty() && this.currentLocaleInfo != null) {
            ...
            if (langCode.equals(this.currentLocaleInfo.shortName)) {
                ...
                try {
                    ...
                    if (BuildVars.LOGS_ENABLED) {
                        FileLog.m1221d("save locale file to " + finalFile);
                    }
                    ...
                    writer.write("</resources>");
                    writer.close();
                    AndroidUtilities.runOnUIThread(new LocaleController$$Lambda$3(this, langCode, difference, getLocaleFileStrings(finalFile)));
                } catch (Exception e) {
                }
            }
        }
    ...
    final /* synthetic */ void lambda$saveRemoteLocaleStrings$3$LocaleController(String langCode, TLRPC$TL_langPackDifference difference, HashMap valuesToSet) {
        LocaleController$LocaleInfo localeInfo = getLanguageFromDict(langCode);
        if (localeInfo != null) {
            localeInfo.version = difference.version;
        }
        saveOtherLanguages();
        if (this.currentLocaleInfo == null || !this.currentLocaleInfo.isLocal()) {
            try {
                Locale newLocale;
                String[] args = localeInfo.shortName.split("_");
                if (args.length == 1) {
    ...
    public void loadRemoteLanguages(int currentAccount) {
        if (!this.loadingRemoteLanguages) {
            ...
            ConnectionsManager.getInstance(currentAccount).sendRequest(new TLRPC$TL_langpack_getLanguages(), new LocaleController$$Lambda$4(this, currentAccount), 8);
        }
    }

    final /* synthetic */ void lambda$loadRemoteLanguages$5$LocaleController(int currentAccount, TLObject response, TLRPC$TL_error error) {
        if (response != null) {
            AndroidUtilities.runOnUIThread(new LocaleController$$Lambda$9(this, response, currentAccount));
        }
    }

    final /* synthetic */ void lambda$null$4$LocaleController(TLObject response, int currentAccount) {
        int a;
        this.loadingRemoteLanguages = false;
        TLRPC$Vector res = (TLRPC$Vector) response;
        HashMap<String, LocaleController$LocaleInfo> remoteLoaded = new HashMap();
        this.remoteLanguages.clear();
        for (a = 0; a < res.objects.size(); a++) {
            TLRPC$TL_langPackLanguage language = (TLRPC$TL_langPackLanguage) res.objects.get(a);
            if (BuildVars.LOGS_ENABLED) {
                FileLog.m1221d("loaded lang " + language.name);
            }
            LocaleController$LocaleInfo localeInfo = new LocaleController$LocaleInfo();
            ...
            LocaleController$LocaleInfo existing = getLanguageFromDict(localeInfo.getKey());
            if (existing == null) {
                this.languages.add(localeInfo);
                this.languagesDict.put(localeInfo.getKey(), localeInfo);
                existing = localeInfo;
        ...
        a = 0;
        while (a < this.languages.size()) {
            LocaleController$LocaleInfo info = (LocaleController$LocaleInfo) this.languages.get(a);
            if (!info.isBuiltIn() && info.isRemote() && ((LocaleController$LocaleInfo) remoteLoaded.get(info.getKey())) == null) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1221d("remove lang " + info.getKey());
                }
                this.languages.remove(a);
                this.languagesDict.remove(info.getKey());
                a--;
                if (info == this.currentLocaleInfo) {
                    if (this.systemDefaultLocale.getLanguage() != null) {
                        info = getLanguageFromDict(this.systemDefaultLocale.getLanguage());
                    }
                    if (info == null) {
    ...
    private void applyRemoteLanguage(LocaleController$LocaleInfo localeInfo, boolean force, int currentAccount) {
        if (localeInfo == null) {
            return;
        }
        ...
        if (localeInfo.version == 0 || force) {
            ...
            ConnectionsManager.getInstance(currentAccount).sendRequest(req, new LocaleController$$Lambda$6(this, currentAccount), 8);
            return;
        }
        TLRPC$TL_langpack_getDifference req2 = new TLRPC$TL_langpack_getDifference();
        req2.from_version = localeInfo.version;
        ConnectionsManager.getInstance(currentAccount).sendRequest(req2, new LocaleController$$Lambda$5(this, currentAccount), 8);
    }

    final /* synthetic */ void lambda$applyRemoteLanguage$7$LocaleController(int currentAccount, TLObject response, TLRPC$TL_error error) {
        if (response != null) {
            AndroidUtilities.runOnUIThread(new LocaleController$$Lambda$8(this, response, currentAccount));
        }
    }

    final /* synthetic */ void lambda$null$6$LocaleController(TLObject response, int currentAccount) {
        saveRemoteLocaleStrings((TLRPC$TL_langPackDifference) response, currentAccount);
    }

    final /* synthetic */ void lambda$applyRemoteLanguage$9$LocaleController(int currentAccount, TLObject response, TLRPC$TL_error error) {
        if (response != null) {
            AndroidUtilities.runOnUIThread(new LocaleController$$Lambda$7(this, response, currentAccount));
        }
    }

    final /* synthetic */ void lambda$null$8$LocaleController(TLObject response, int currentAccount) {
        saveRemoteLocaleStrings((TLRPC$TL_langPackDifference) response, currentAccount);
    }

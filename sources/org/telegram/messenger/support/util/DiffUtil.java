...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new C09101();

    /* renamed from: org.telegram.messenger.support.util.DiffUtil$1 */
    static class C09101 implements Comparator<Snake> {
        C09101() {
        }

        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f781x - o2.f781x;
            return cmpX == 0 ? o1.f782y - o2.f782y : cmpX;
        }
    }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f781x != 0 || firstSnake.f782y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f781x + snake.size;
                int endY = snake.f782y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f781x + j;
                    int newItemPos = snake.f782y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f781x + snakeSize;
                int endY = snake.f782y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f781x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f781x + i, 1, this.mCallback.getChangePayload(snake.f781x + i, snake.f782y + i));
                    }
                }
                posOld = snake.f781x;
                posNew = snake.f782y;
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f781x += range.oldListStart;
                snake.f782y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f781x;
                    left.newListEnd = snake.f782y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f781x - 1;
                    left.newListEnd = snake.f782y;
                } else {
                    left.oldListEnd = snake.f781x;
                    left.newListEnd = snake.f782y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f781x + snake.size;
                    right.newListStart = snake.f782y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = (snake.f781x + snake.size) + 1;
                    right.newListStart = snake.f782y + snake.size;
                } else {
                    right.oldListStart = snake.f781x + snake.size;
                    right.newListStart = (snake.f782y + snake.size) + 1;
                }
                stack.add(right);
            } else {
                rangePool.add(range);
        ...
        while (d <= dLimit) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f781x = backward[kOffset + k];
                    outSnake.f782y = outSnake.f781x - k;
            ...
            while (k <= d) {
                ...
                if (checkInFwd || k + delta < (-d) || k + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake = new Snake();
                    outSnake.f781x = backward[kOffset + backwardK];
                    outSnake.f782y = outSnake.f781x - backwardK;

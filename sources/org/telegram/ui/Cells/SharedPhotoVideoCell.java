...
public class SharedPhotoVideoCell extends FrameLayout {
    ...
    class C11492 implements OnLongClickListener {
        C11492() {
        }
    ...
    private class PhotoVideoView extends FrameLayout {
        ...
        public PhotoVideoView(Context context) {
            ...
            this.imageView1.setImageResource(R.drawable.ic_video);
            this.videoInfoContainer.addView(this.imageView1, LayoutHelper.createFrame(-2, -2, 19));
            ...
            this.videoTextView.setTypeface(TurboUtils.getTurboTypeFace());
        }
    ...
    public SharedPhotoVideoCell(Context context) {
        ...
        for (int a = 0; a < 6; a++) {
            ...
            this.photoVideoViews[a].setOnClickListener(new C11481());
            this.photoVideoViews[a].setOnLongClickListener(new C11492());
        }
    }
        ...
        if (messageObject != null) {
            ...
            if (messageObject.isVideo()) {
                ...
                for (b = 0; b < messageObject.getDocument().attributes.size(); b++) {
                    attribute = (TLRPC$DocumentAttribute) messageObject.getDocument().attributes.get(b);
                ...
                photoVideoView.imageView1.setVisibility(8);
                photoVideoView.videoInfoContainer.setVisibility(0);
                duration = 0;
                for (b = 0; b < messageObject.getDocument().attributes.size(); b++) {
                    attribute = (TLRPC$DocumentAttribute) messageObject.getDocument().attributes.get(b);
                ...
                seconds = duration - ((duration / 60) * 60);
                photoVideoView.videoTextView.setText("GIF, " + String.format("%d:%02d", new Object[]{Integer.valueOf(duration / 60), Integer.valueOf(seconds)}));
                ...
                if (messageObject.mediaExists || DownloadController.getInstance(UserConfig.selectedAccount).canDownloadMedia(messageObject) || FileLoader.getInstance(UserConfig.selectedAccount).isLoadingFile(attachFileName)) {
                    photoVideoView.imageView.setImage(messageObject.messageOwner.media.document, null, null, ApplicationLoader.applicationContext.getResources().getDrawable(R.drawable.photo_placeholder_in), null, photoSize.location, "b", null, messageObject.messageOwner.media.document.size);
                    return;
                } else {

...
public class DialogCell extends BaseCell implements PhotoViewerProvider {
    private static GradientDrawable statusDrawable;
    ...
    public DialogCell(Context context, boolean needCheck) {
        ...
        statusDrawable = new GradientDrawable();
        statusDrawable.setColor(-3355444);
        statusDrawable.setCornerRadius((float) AndroidUtilities.dp(13.0f));
        statusDrawable.setStroke(AndroidUtilities.dp(2.0f), Theme.getColor(Theme.key_windowBackgroundWhite));
    }
        ...
        if (this.customDialog != null) {
            ...
            if (this.encryptedChat != null) {
                ...
                if (this.user != null) {
                    ...
                    if (this.user.verified || this.user.id == UserConfig.getInstance(this.currentAccount).getClientUserId()) {
                        z = true;
                    } else {
                        z = false;
                    }
                ...
                if (this.draftMessage != null) {
                    ...
                    if (this.encryptedChat instanceof TLRPC$TL_encryptedChatRequested) {
                        ...
                        if (this.user == null || this.user.first_name == null) {
                            messageString = LocaleController.formatString("AwaitingEncryption", R.string.AwaitingEncryption, new Object[]{""});
                        } else {
                            messageString = LocaleController.formatString("AwaitingEncryption", R.string.AwaitingEncryption, new Object[]{this.user.first_name});
                        }
                        ...
                        if (this.encryptedChat.admin_id != UserConfig.getInstance(this.currentAccount).getClientUserId()) {
                            ...
                            messageString = LocaleController.formatString("EncryptedChatStartedOutgoing", R.string.EncryptedChatStartedOutgoing, new Object[]{""});
                        } else {
                            messageString = LocaleController.formatString("EncryptedChatStartedOutgoing", R.string.EncryptedChatStartedOutgoing, new Object[]{this.user.first_name});
                        }
                    }
                } else {
                    messageString = "";
            ...
            if (TurboConfig.chatLockPasscode.length() > 0 && TurboConfig.containValue("lock_chat" + this.currentDialogId)) {
                messageString = LocaleController.getString("TurboProtected", R.string.TurboProtected);
            }
            if (this.draftMessage != null) {
            ...
            if (this.message == null) {
                ...
                if (!this.message.isOut() || this.draftMessage != null || !showChecks) {
                    ...
                    z = !this.message.isUnread() || (ChatObject.isChannel(this.chat) && !this.chat.megagroup);
                    this.drawCheck1 = z;
                    this.drawCheck2 = true;
                    this.drawClock = false;
                    this.drawError = false;
    ...
    public void checkCurrentDialogIndex() {
        if (this.index < getDialogsArray().size()) {
            ...
            TLRPC$DraftMessage newDraftMessage = DataQuery.getInstance(this.currentAccount).getDraft(this.currentDialogId);
    ...
    public void update(int mask) {
        if (this.customDialog != null) {
            ...
            boolean z;
            if (this.isDialogCell) {
                dialog = (TLRPC$TL_dialog) MessagesController.getInstance(this.currentAccount).dialogs_dict.get(this.currentDialogId);
            ...
            if (lower_id == 0) {
                ...
                if (!(this.isDialogCell || this.chat == null || this.chat.migrated_to == null)) {
                    TLRPC$Chat chat2 = MessagesController.getInstance(this.currentAccount).getChat(Integer.valueOf(this.chat.migrated_to.channel_id));
                    if (chat2 != null) {
                        this.chat = chat2;
                    }
                }
    ...
    private void setStatus(Canvas canvas) {
        if (this.user != null && this.chat == null) {
            boolean isBot = this.user != null && this.user.bot;
            if (!isBot && TurboConfig.contactStatus && statusDrawable != null) {
                int x;
                int y = AndroidUtilities.dp(46.0f);
                if (LocaleController.isRTL) {
                    x = getMeasuredWidth() - AndroidUtilities.dp(62.0f);
                } else {
                    x = AndroidUtilities.dp(45.0f);
                }
                statusDrawable.setBounds(x, y, AndroidUtilities.dp(17.0f) + x, AndroidUtilities.dp(17.0f) + y);
                if ((this.user != null && this.user.status != null && this.user.status.expires > ConnectionsManager.getInstance(this.currentAccount).getCurrentTime()) || MessagesController.getInstance(this.currentAccount).onlinePrivacy.containsKey(Integer.valueOf(this.user.id))) {
                    statusDrawable.setColor(-14032632);
                } else if (this.user == null || this.user.status == null || this.user.status.expires == 0 || UserObject.isDeleted(this.user) || (this.user instanceof TLRPC$TL_userEmpty)) {
                    statusDrawable.setColor(-16777216);
                } else {
                    statusDrawable.setColor(-3355444);
                }
    ...
    public void showAvatar(User u, TLRPC$Chat c) {
        if (u != null) {
            if (u.photo != null && u.photo.photo_big != null) {
                PhotoViewer.getInstance().openPhoto(u.photo.photo_big, this);
            }
        } else if (c != null && this.chat.photo != null && this.chat.photo.photo_big != null) {
            PhotoViewer.getInstance().openPhoto(this.chat.photo.photo_big, this);
        }
    }
    ...
    public PlaceProviderObject getPlaceForPhoto(MessageObject messageObject, TLRPC$FileLocation fileLocation, int index) {
        ...
        if (fileLocation != null) {
            ...
            if (this.user != null) {
                index = this.user.id;
                if (this.user == null || this.user.photo == null || this.user.photo.photo_big == null) {
                    photoBig = null;
                } else {
                    photoBig = this.user.photo.photo_big;
                }
            } else if (this.chat != null) {
                if (this.chat == null || this.chat.photo == null || this.chat.photo.photo_big == null) {
                    photoBig = null;
                } else {
                    photoBig = this.chat.photo.photo_big;
                    index = 0;
                }
            }
            if (photoBig != null && photoBig.local_id == fileLocation.local_id && photoBig.volume_id == fileLocation.volume_id && photoBig.dc_id == fileLocation.dc_id) {
                ...
                placeProviderObject.parentView = this;
                placeProviderObject.imageReceiver = this.avatarImage;
                placeProviderObject.dialogId = index;
                placeProviderObject.thumb = placeProviderObject.imageReceiver.getBitmapSafe();
                placeProviderObject.size = -1;
                placeProviderObject.radius = this.avatarImage.getRoundRadius();
                placeProviderObject.scale = getScaleX();
            }
        }

...
public abstract class BaseLocationAdapter extends SelectionAdapter {
    ...
    final /* synthetic */ void lambda$searchBotUser$1$BaseLocationAdapter(TLObject response, TLRPC$TL_error error) {
        if (response != null) {
    ...
    final /* synthetic */ void lambda$searchPlacesWithQuery$3$BaseLocationAdapter(TLObject response, TLRPC$TL_error error) {
        AndroidUtilities.runOnUIThread(new BaseLocationAdapter$$Lambda$2(this, error, response));
    }

    final /* synthetic */ void lambda$null$2$BaseLocationAdapter(TLRPC$TL_error error, TLObject response) {
        this.currentRequestNum = 0;
        this.searching = false;
        this.places.clear();
        this.iconUrls.clear();
        if (error == null) {
            ...
            for (int a = 0; a < size; a++) {
                ...
                if ("venue".equals(result.type) && (result.send_message instanceof TLRPC$TL_botInlineMessageMediaVenue)) {
                    TLRPC$TL_botInlineMessageMediaVenue mediaVenue = result.send_message;

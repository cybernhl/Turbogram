...
public class ContactsAdapter extends SectionsAdapter {
    ...
    private boolean showOnlines = false;
    ...
    public void setShowOnlines(boolean onlines) {
        this.showOnlines = onlines;
    }

    public void setShowSwitch(boolean state) {
    ...
    public Object getItem(int section, int position) {
        HashMap<String, ArrayList<TLRPC$TL_contact>> usersSectionsDict;
        ArrayList<String> sortedUsersSectionsArray;
        if (this.showOnlines) {
            usersSectionsDict = this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).onlineUsersMutualSectionsDict : ContactsController.getInstance(this.currentAccount).onlineUsersSectionsDict;
            sortedUsersSectionsArray = this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).onlineSortedUsersMutualSectionsArray : ContactsController.getInstance(this.currentAccount).onlineSortedUsersSectionsArray;
        } else {
            usersSectionsDict = this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).usersMutualSectionsDict : ContactsController.getInstance(this.currentAccount).usersSectionsDict;
            if (this.onlyUsers == 2) {
        ...
        if (this.onlyUsers == 0 || this.isAdmin) {
            ...
            if (section - 1 < sortedUsersSectionsArray.size()) {
                ...
                if (position < arr.size()) {
                    return MessagesController.getInstance(this.currentAccount).getUser(Integer.valueOf(((TLRPC$TL_contact) arr.get(position)).user_id));
                }
                return null;
            } else if (!this.needPhonebook || this.showOnlines) {
                return null;
            } else {
    ...
    public boolean isEnabled(int section, int row) {
        ...
        if (this.showOnlines) {
            ...
            usersSectionsDict = this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).usersMutualSectionsDict : ContactsController.getInstance(this.currentAccount).usersSectionsDict;
            sortedUsersSectionsArray = this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).sortedUsersMutualSectionsArray : ContactsController.getInstance(this.currentAccount).sortedUsersSectionsArray;
        }
        if (this.onlyUsers == 0 || this.isAdmin) {
            if (section == 0) {
    ...
    public int getSectionCount() {
        ArrayList<String> sortedUsersSectionsArray = this.showOnlines ? this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).onlineSortedUsersMutualSectionsArray : ContactsController.getInstance(this.currentAccount).onlineSortedUsersSectionsArray : this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).sortedUsersMutualSectionsArray : ContactsController.getInstance(this.currentAccount).sortedUsersSectionsArray;
        ...
        if (!this.needPhonebook || this.showOnlines) {
            return count;
        }
        return count + 1;
    }
    ...
    public View getSectionHeaderView(int section, View view) {
        ...
        if (this.showOnlines) {
            if (this.onlyUsers == 2) {
                HashMap<String, ArrayList<TLRPC$TL_contact>> usersSectionsDict = ContactsController.getInstance(this.currentAccount).onlineUsersMutualSectionsDict;
            } else {
                hashMap = ContactsController.getInstance(this.currentAccount).onlineUsersSectionsDict;
            }
            sortedUsersSectionsArray = this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).onlineSortedUsersMutualSectionsArray : ContactsController.getInstance(this.currentAccount).onlineSortedUsersSectionsArray;
        } else {
            if (this.onlyUsers == 2) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case 2:
                ...
                view = new ContactsGraySectionCell(this.mContext);
                ((ContactsGraySectionCell) view).setText(LocaleController.getString("Contacts", R.string.Contacts).toUpperCase());
                break;
            default:
                float f2;
    ...
    public void onBindViewHolder(int section, int position, ViewHolder holder) {
        switch (holder.getItemViewType()) {
            case 0:
                ...
                User user = MessagesController.getInstance(this.currentAccount).getUser(Integer.valueOf(((TLRPC$TL_contact) ((ArrayList) usersSectionsDict.get(sortedUsersSectionsArray.get(section - i))).get(position)).user_id));
    ...
    public String getLetter(int position) {
        ArrayList<String> sortedUsersSectionsArray = this.onlyUsers == 2 ? ContactsController.getInstance(this.currentAccount).onlineSortedUsersMutualSectionsArray : ContactsController.getInstance(this.currentAccount).onlineSortedUsersSectionsArray;
    ...
    public ArrayList<User> getAllItem(int count) {
        ArrayList<User> users = new ArrayList();
        for (int a = 0; a < count; a++) {
            int section = getSectionForPosition(a);
            int row = getPositionInSectionForPosition(a);
            if (row >= 0 && section >= 0) {
                User user = (User) getItem(section, row);
                if (user != null) {
                    users.add(user);
                }
            }

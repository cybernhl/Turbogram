...
public class MentionsAdapter extends SelectionAdapter {
    ...
    private SendMessagesHelper$LocationProvider locationProvider = new SendMessagesHelper$LocationProvider(new C10141()) {
        public void stop() {
            super.stop();
    ...
    class C10141 implements LocationProviderDelegate {
        C10141() {
        }
    ...
    class C10163 implements SearchAdapterHelperDelegate {
        C10163() {
        }
    ...
    public MentionsAdapter(Context context, boolean darkTheme, long did, MentionsAdapterDelegate mentionsAdapterDelegate) {
        ...
        this.searchAdapterHelper.setDelegate(new C10163());
    }
    ...
    private void searchForContextBot(String username, String query) {
        if (this.foundContextBot == null || this.foundContextBot.username == null || !this.foundContextBot.username.equals(username) || this.searchingContextQuery == null || !this.searchingContextQuery.equals(query)) {
            ...
            this.contextQueryRunnable = new Runnable() {
                public void run() {
                    if (MentionsAdapter.this.contextQueryRunnable == this) {
                        ...
                        if (MentionsAdapter.this.foundContextBot == null && !MentionsAdapter.this.noUserName) {
                            ...
                            MentionsAdapter.this.searchForContextBotResults(true, MentionsAdapter.this.foundContextBot, str, "");
                        }
                    }
                }

                final /* synthetic */ void lambda$run$1$MentionsAdapter$4(String username, MessagesController messagesController, MessagesStorage messagesStorage, TLObject response, TLRPC$TL_error error) {
                ...
                final /* synthetic */ void lambda$null$0$MentionsAdapter$4(String username, TLRPC$TL_error error, TLObject response, MessagesController messagesController, MessagesStorage messagesStorage) {
    ...
    final /* synthetic */ void lambda$searchForContextBotResults$4$MentionsAdapter(String query, boolean cache, User user, String offset, MessagesStorage messagesStorage, String key, TLObject response, TLRPC$TL_error error) {
    ...
    public void searchUsernameOrHashtag(String text, int position, ArrayList<MessageObject> messageObjects, boolean usernameOnly) {
        ...
        if (this.foundContextBot == null) {
            ...
            if (foundType == -1) {
                ...
                ArrayList<TLRPC$TL_topPeer> inlineBots = DataQuery.getInstance(this.currentAccount).inlineBots;
                if (!usernameOnly && this.needBotContext && dogPostion == 0 && !inlineBots.isEmpty()) {
                    int count = 0;
                    for (a = 0; a < inlineBots.size(); a++) {
                        user = messagesController.getUser(Integer.valueOf(((TLRPC$TL_topPeer) inlineBots.get(a)).peer.user_id));
                        if (user != null) {
                ...
                if (!(chat == null || this.info == null || this.info.participants == null || (ChatObject.isChannel(chat) && !chat.megagroup))) {
                    for (a = 0; a < this.info.participants.participants.size(); a++) {
                        user = messagesController.getUser(Integer.valueOf(((TLRPC$ChatParticipant) this.info.participants.participants.get(a)).user_id));
                ...
                if (chat != null && chat.megagroup && usernameString.length() > 0) {
                    ...
                    C10185 c10185 = new Runnable() {
                        ...
                        final /* synthetic */ void lambda$run$1$MentionsAdapter$5(int currentReqId, MessagesController messagesController, TLObject response, TLRPC$TL_error error) {
                            AndroidUtilities.runOnUIThread(new MentionsAdapter$5$$Lambda$1(this, currentReqId, error, response, messagesController));
                        }

                        final /* synthetic */ void lambda$null$0$MentionsAdapter$5(int currentReqId, TLRPC$TL_error error, TLObject response, MessagesController messagesController) {
                    ...
                    this.searchGlobalRunnable = c10185;
                    AndroidUtilities.runOnUIThread(c10185, 200);
                }
                ...
                for (int b = 0; b < this.botInfo.size(); b++) {
                    TLRPC$BotInfo info = (TLRPC$BotInfo) this.botInfo.valueAt(b);
                    for (a = 0; a < info.commands.size(); a++) {
                        TLRPC$TL_botCommand botCommand = (TLRPC$TL_botCommand) info.commands.get(a);

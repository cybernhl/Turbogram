...
public class ImageUpdater implements NotificationCenter$NotificationCenterDelegate, PhotoEditActivityDelegate {
    ...
    public interface ImageUpdaterDelegate {
        void didUploadedPhoto(TLRPC$InputFile tLRPC$InputFile, TLRPC$PhotoSize tLRPC$PhotoSize, TLRPC$PhotoSize tLRPC$PhotoSize2, TLRPC$TL_secureFile tLRPC$TL_secureFile);
    }

    /* renamed from: org.telegram.ui.Components.ImageUpdater$1 */
    class C14561 implements PhotoAlbumPickerActivityDelegate {
        C14561() {
        }

        public void didSelectPhotos(ArrayList<SendMessagesHelper$SendingMediaInfo> photos) {
            if (!photos.isEmpty()) {
                ImageUpdater.this.processBitmap(ImageLoader.loadBitmap(((SendMessagesHelper$SendingMediaInfo) photos.get(0)).path, null, 800.0f, 800.0f, true));
            }
        }
                ...
                if (VERSION.SDK_INT < 23 || this.parentFragment.getParentActivity().checkSelfPermission("android.permission.CAMERA") == 0) {
                    ...
                    if (image != null) {
                        if (VERSION.SDK_INT >= 24) {
                            takePictureIntent.putExtra("output", FileProvider.getUriForFile(this.parentFragment.getParentActivity(), "com.baranak.turbogramf.provider", image));
    ...
    public void openGallery() {
        if (this.parentFragment != null) {
            if (VERSION.SDK_INT < 23 || this.parentFragment == null || this.parentFragment.getParentActivity() == null || this.parentFragment.getParentActivity().checkSelfPermission("android.permission.READ_EXTERNAL_STORAGE") == 0) {
                ...
                fragment.setDelegate(new C14561());
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 13) {
            ...
            int orientation = 0;
            try {
                switch (new ExifInterface(this.currentPicturePath).getAttributeInt(ExifInterface.TAG_ORIENTATION, 1)) {
                    case 3:
                        orientation = 180;
                        break;
                    case 6:
    ...
    private void processBitmap(Bitmap bitmap) {
        if (bitmap != null) {
            if (this.encryptedDocument) {
                this.bigPhoto = ImageLoader.scaleAndSaveImage(bitmap, 2048.0f, 2048.0f, 89, false, 320, 320);
                if (this.delegate != null) {
                    TLRPC$TL_secureFile secureFile = new TLRPC$TL_secureFile();
                    secureFile.dc_id = (int) this.bigPhoto.location.volume_id;
                    secureFile.id = (long) this.bigPhoto.location.local_id;
            ...
            if (this.bigPhoto != null && this.smallPhoto != null) {
                if (!this.returnOnly) {
                    ...
                    this.delegate.didUploadedPhoto(null, this.smallPhoto, this.bigPhoto, null);
                }
            }
        }
    }
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        ...
        if (id == NotificationCenter.FileDidUpload) {
            ...
            if (this.uploadingImage != null && location.equals(this.uploadingImage)) {
                ...
                if (this.delegate != null) {
                    this.delegate.didUploadedPhoto((TLRPC$InputFile) args[1], this.smallPhoto, this.bigPhoto, null);
                }

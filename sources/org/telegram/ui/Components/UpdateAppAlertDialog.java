...
public class UpdateAppAlertDialog extends AlertDialog implements NotificationCenter$NotificationCenterDelegate {
    ...
    class C16711 implements OnClickListener {
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (UpdateAppAlertDialog.this.appUpdate.document instanceof TLRPC$TL_document) {
    ...
    class C16722 implements OnClickListener {
        C16722() {
        }
    ...
    public UpdateAppAlertDialog(Activity activity, TLRPC$TL_help_appUpdate update, int account) {
        ...
        setPositiveButton(LocaleController.getString("UpdateNow", R.string.UpdateNow), new C16711());
        setNeutralButton(LocaleController.getString("Later", R.string.Later), new C16722());
        this.radialProgressView = new FrameLayout(this.parentActivity) {
            protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
                super.onLayout(changed, left, top, right, bottom);
                int width = right - left;
                int height = bottom - top;
                int w = AndroidUtilities.dp(24.0f);
                int l = (width - w) / 2;
                int t = ((height - w) / 2) + AndroidUtilities.dp(2.0f);

...
public class PhotoFilterBlurControl extends FrameLayout {
            ...
            case 5:
                if (event.getPointerCount() == 1) {
                    if (this.checkForMoving && !this.isMoving) {
                        ...
                        Point delta = new Point(locationX - centerPoint.f828x, locationY - centerPoint.f829y);
                        float radialDistance = (float) Math.sqrt((double) ((delta.f828x * delta.f828x) + (delta.f829y * delta.f829y)));
                        ...
                        if (this.type == 0) {
                            float distance = (float) Math.abs((((double) delta.f828x) * Math.cos(((double) degreesToRadians(this.angle)) + 1.5707963267948966d)) + (((double) delta.f829y) * Math.sin(((double) degreesToRadians(this.angle)) + 1.5707963267948966d)));
    ...
    private void handlePan(int state, MotionEvent event) {
        ...
        Point delta = new Point(locationX - actualCenterPoint.f828x, locationY - actualCenterPoint.f829y);
        float radialDistance = (float) Math.sqrt((double) ((delta.f828x * delta.f828x) + (delta.f829y * delta.f829y)));
            ...
            case 2:
                    ...
                    if (this.type == 1) {
                        switch (this.activeControl) {
                            case BlurViewActiveControlCenter:
                                ...
                                point = new Point(Math.max(actualArea.f830x, Math.min(actualArea.f830x + actualArea.width, this.startCenterPoint.f828x + translationX)), Math.max(actualArea.f831y, Math.min(actualArea.f831y + actualArea.height, this.startCenterPoint.f829y + translationY)));
                                this.centerPoint = new Point((point.f828x - actualArea.f830x) / this.actualAreaSize.width, ((point.f829y - actualArea.f831y) + ((this.actualAreaSize.width - this.actualAreaSize.height) / 2.0f)) / this.actualAreaSize.width);
                                break;
                ...
                switch (this.activeControl) {
                    ...
                    case BlurViewActiveControlRotation:
                        ...
                        boolean right = locationX > actualCenterPoint.f828x;
                        boolean bottom = locationY > actualCenterPoint.f829y;
                        if (right || bottom) {
                            if (!right || bottom) {
                                if (right && bottom) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float innerRadius = getActualInnerRadius();
        float outerRadius = getActualOuterRadius();
        canvas.translate(centerPoint.f828x, centerPoint.f829y);
        int i;
        Canvas canvas2;
        if (this.type == 0) {
            float f;
    ...
    private Point getActualCenterPoint() {
        return new Point((this.centerPoint.f828x * this.actualAreaSize.width) + ((((float) getWidth()) - this.actualAreaSize.width) / 2.0f), ((((float) (VERSION.SDK_INT >= 21 ? AndroidUtilities.statusBarHeight : 0)) + ((((float) getHeight()) - this.actualAreaSize.height) / 2.0f)) - ((this.actualAreaSize.width - this.actualAreaSize.height) / 2.0f)) + (this.centerPoint.f829y * this.actualAreaSize.width));
    }

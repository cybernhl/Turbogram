...
public class CropView extends FrameLayout implements AreaViewListener, CropGestureListener {
    ...
    class C13921 implements OnPreDrawListener {
        C13921() {
        }
    ...
    class C13987 implements OnCancelListener {
        C13987() {
        }
    ...
    private class CropState {
        ...
        private boolean hasChanges() {
            return Math.abs(this.f817x) > CropView.EPSILON || Math.abs(this.f818y) > CropView.EPSILON || Math.abs(this.scale - this.minimumScale) > CropView.EPSILON || Math.abs(this.rotation) > CropView.EPSILON || Math.abs(this.orientation) > CropView.EPSILON;
        }
    ...
    public void setBitmap(Bitmap b, int rotation, boolean fform) {
        ...
        this.imageView.getViewTreeObserver().addOnPreDrawListener(new C13921());
    ...
    public void showAspectRatioDialog() {
        if (this.areaView.getLockAspectRatio() > 0.0f) {
            ...
            dialog.setOnCancelListener(new C13987());
            dialog.show();
        }
    }

...
public class EntityView extends FrameLayout {
    ...
    class C15101 extends SimpleOnGestureListener {
        C15101() {
        }
    ...
    public class SelectionView extends FrameLayout {
        ...
        protected void updatePosition() {
            ...
            layoutParams.leftMargin = ((int) bounds.f830x) + EntityView.this.offsetX;
            layoutParams.topMargin = ((int) bounds.f831y) + EntityView.this.offsetY;
                ...
                case 2:
                    if (this.currentHandle != 3) {
                        if (this.currentHandle != 0) {
                            ...
                            float delta = (float) ((((double) translation.f828x) * Math.cos((double) radAngle)) + (((double) translation.f829y) * Math.sin((double) radAngle)));
    ...
    public EntityView(Context context, Point pos) {
        super(context);
        this.position = pos;
        this.gestureDetector = new GestureDetector(context, new C15101());
    }
    ...
    private boolean onTouchMove(float x, float y) {
        ...
        if (((float) Math.hypot((double) translation.f828x, (double) translation.f829y)) <= (this.hasPanned ? 6.0f : 16.0f)) {
            return false;
        }
        pan(translation);
    ...
    public void pan(Point translation) {
        ...
        point.f828x += translation.f828x;
        point = this.position;
        point.f829y += translation.f829y;
        updatePosition();
    }

    protected void updatePosition() {
        float halfHeight = ((float) getHeight()) / 2.0f;
        setX(this.position.f828x - (((float) getWidth()) / 2.0f));
        setY(this.position.f829y - halfHeight);
        updateSelectionView();
    }

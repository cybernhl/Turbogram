    ...
    public void updateOnlineCount() {
        if (this.parentFragment != null) {
            ...
            if (info != null) {
                ...
                if ((info instanceof TLRPC$TL_chatFull) || ((info instanceof TLRPC$TL_channelFull) && info.participants_count <= Callback.DEFAULT_DRAG_ANIMATION_DURATION && info.participants != null)) {
                    for (int a = 0; a < info.participants.participants.size(); a++) {
                        User user = MessagesController.getInstance(this.currentAccount).getUser(Integer.valueOf(((TLRPC$ChatParticipant) info.participants.participants.get(a)).user_id));

...
public class RadialProgress {
    ...
    public RadialProgress(View parentView) {
        ...
        progressTextPaint = new Paint(1);
        progressTextPaint.setColor(-1);
        progressTextPaint.setTextSize((float) AndroidUtilities.dp(9.0f));
        progressTextPaint.setFakeBoldText(false);
        progressTextPaint.setTextAlign(Align.CENTER);
        progressTextPaint.setTypeface(TurboUtils.getTurboTypeFace());
    }
        ...
        if (!this.drawMiniProgress || this.currentDrawable == null) {
            ...
            if (this.currentWithRound || this.previousWithRound) {
                ...
                if (this.currentDrawable != null && progressTextPaint != null && this.currentProgress < 1.0f && this.docSize > 0) {
                    ...
                    if (this.docType != 0) {
                        string = " | " + String.format(((float) this.docSize) * this.currentProgress < 1.04857E8f ? "%.1f" : "%.0f", new Object[]{Float.valueOf(this.currentProgress * 100.0f)}) + '%';
                    } else {
                        string = "";
                    }
                    String string2 = append.append(string).toString();
                    int n = (int) this.progressRect.left;
                    int n2 = this.currentDrawable.getIntrinsicWidth() / 2;
                    if (this.docType == 14) {
                        n3 = (float) ((n + n2) + AndroidUtilities.dp(1.0f));
                        n4 = (int) this.progressRect.bottom;
                    } else {
                        n3 = (float) ((n + n2) + AndroidUtilities.dp(1.0f));
                        n4 = (int) this.progressRect.bottom;
                    }
                    ...
                    progressTextPaint.getTextBounds(string2, 0, string2.length(), bounds);
                    float width = (float) bounds.width();
                    if (this.docType == 1 || this.docType == 3 || this.docType == 8) {
                        progressTextPaint.setColor(this.progressColor);
                        int mid = (int) ((this.progressRect.left + this.progressRect.right) / 2.0f);
                        int left = mid - ((int) ((((float) AndroidUtilities.dp(6.0f)) + width) / 2.0f));
                        int top = ((int) this.progressRect.bottom) + AndroidUtilities.dp(2.0f);
                        int right = mid + ((int) ((((float) AndroidUtilities.dp(6.0f)) + width) / 2.0f));
                        int bottom = ((int) this.progressRect.bottom) + AndroidUtilities.dp(18.0f);
                        RectF rect = new RectF();
                        rect.set((float) left, (float) top, (float) right, (float) bottom);
                        canvas.drawRoundRect(rect, (float) AndroidUtilities.dp(4.0f), (float) AndroidUtilities.dp(4.0f), Theme.chat_timeBackgroundPaint);
                    }
                    canvas.drawText(string2, n3, (float) (AndroidUtilities.dp(n5) + n4), progressTextPaint);
                }
    ...
    public void setSizeAndType(long docSize, int docType) {
        float textSize = 9.0f;
        this.docSize = docSize;
        this.docType = docType;
        if (!(docType == 9 || docType == 14)) {
            textSize = 12.0f;
        }
        progressTextPaint.setTextSize((float) AndroidUtilities.dp(textSize));
    }
}

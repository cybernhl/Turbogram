        ...
        public void draw(Canvas canvas) {
            switch (this.type) {
                case 0:
                    ...
                    canvas.drawPoint(this.f832x, this.f833y, SnowflakesEffect.this.particlePaint);
                    return;
                default:
                    ...
                    for (int a = 0; a < 6; a++) {
                        ...
                        float cx = x1 * 0.66f;
                        float cy = y1 * 0.66f;
                        canvas.drawLine(this.f832x, this.f833y, this.f832x + x1, this.f833y + y1, SnowflakesEffect.this.particleThinPaint);
                        float angle2 = (float) (((double) angle) - 1.5707963267948966d);
                        canvas.drawLine(this.f832x + cx, this.f833y + cy, this.f832x + ((float) ((Math.cos((double) angle2) * ((double) px1)) - (Math.sin((double) angle2) * ((double) py1)))), this.f833y + ((float) ((Math.sin((double) angle2) * ((double) px1)) + (Math.cos((double) angle2) * ((double) py1)))), SnowflakesEffect.this.particleThinPaint);
                        canvas.drawLine(this.f832x + cx, this.f833y + cy, this.f832x + ((float) (((-Math.cos((double) angle2)) * ((double) px1)) - (Math.sin((double) angle2) * ((double) py1)))), this.f833y + ((float) (((-Math.sin((double) angle2)) * ((double) px1)) + (Math.cos((double) angle2) * ((double) py1)))), SnowflakesEffect.this.particleThinPaint);
                        angle += 1.0471976f;
                    }
                    return;
    ...
    private void updateParticles(long dt) {
        ...
        while (a < count) {
            ...
            if (particle.currentTime >= particle.lifeTime) {
                ...
                particle.f832x += ((particle.vx * particle.velocity) * ((float) dt)) / 500.0f;
                particle.f833y += ((particle.vy * particle.velocity) * ((float) dt)) / 500.0f;
    ...
    public void onDraw(View parent, Canvas canvas) {
        if (parent != null && canvas != null) {
            ...
            if (Utilities.random.nextFloat() > 0.7f && this.particles.size() < 100) {
                ...
                newParticle.f832x = cx;
                newParticle.f833y = cy;
                newParticle.vx = vx;
                newParticle.vy = vy;

...
public class NotificationsSettingsActivity extends BaseFragment implements NotificationCenter$NotificationCenterDelegate {
    ...
    class C19141 extends ActionBarMenuOnItemClick {
        C19141() {
        }
                ...
                case 2:
                    ...
                    if (position == NotificationsSettingsActivity.this.resetNotificationsRow) {
                        ...
                        for (a = 0; a < size; a++) {
                            ...
                            User user;
                            if (lower_id == 0) {
                                TLRPC$EncryptedChat encryptedChat = MessagesController.getInstance(NotificationsSettingsActivity.this.currentAccount).getEncryptedChat(Integer.valueOf(high_id));
                                ...
                                TLRPC$Chat chat = MessagesController.getInstance(NotificationsSettingsActivity.this.currentAccount).getChat(Integer.valueOf(-lower_id));
                                if (chat != null) {
                                    name = chat.title;
                                }
                            }
    ...
    final /* synthetic */ void lambda$onFragmentCreate$1$NotificationsSettingsActivity() {
        ...
        if (waitingForLoadExceptions.size() != 0) {
            ...
            int size = chats.size();
            for (a = 0; a < size; a++) {
                chat = (TLRPC$Chat) chats.get(a);
                if (!(chat.left || chat.kicked || chat.migrated_to != null)) {
                    waitingForLoadExceptions.remove((long) (-chat.id));
                }
            }
            size = users.size();
            for (a = 0; a < size; a++) {
            ...
            for (a = 0; a < size; a++) {
                waitingForLoadExceptions.remove(((long) ((TLRPC$EncryptedChat) encryptedChats.get(a)).id) << 32);
            }
    ...
    public View createView(Context context) {
        ...
        this.actionBar.setActionBarMenuOnItemClick(new C19141());
    ...
    final /* synthetic */ void lambda$createView$10$NotificationsSettingsActivity(View view, int position) {
        ...
        if (getParentActivity() != null) {
            ...
            if (position == this.messageAlertRow || position == this.groupAlertRow) {
                ...
                r0.setMessage(LocaleController.getString("ResetNotificationsAlert", R.string.ResetNotificationsAlert));
                r0.setTitle(LocaleController.getString("AppName", R.string.AppName));
                r0.setPositiveButton(LocaleController.getString("Reset", R.string.Reset), new NotificationsSettingsActivity$$Lambda$3(this));
                ...
                r0 = new Builder(getParentActivity());
                r0.setTitle(LocaleController.getString("RepeatNotifications", R.string.RepeatNotifications));
                r0.setItems(new CharSequence[]{LocaleController.getString("RepeatDisabled", R.string.RepeatDisabled), LocaleController.formatPluralString("Minutes", 5), LocaleController.formatPluralString("Minutes", 10), LocaleController.formatPluralString("Minutes", 30), LocaleController.formatPluralString("Hours", 1), LocaleController.formatPluralString("Hours", 2), LocaleController.formatPluralString("Hours", 4)}, new NotificationsSettingsActivity$$Lambda$8(this, position));
                r0.setNegativeButton(LocaleController.getString("Cancel", R.string.Cancel), null);
                showDialog(r0.create());
            }
    ...
    final /* synthetic */ void lambda$null$4$NotificationsSettingsActivity(DialogInterface dialogInterface, int i) {
        if (!this.reseting) {
            this.reseting = true;
            ConnectionsManager.getInstance(this.currentAccount).sendRequest(new TLRPC$TL_account_resetNotifySettings(), new NotificationsSettingsActivity$$Lambda$9(this));
        }
    }

    final /* synthetic */ void lambda$null$3$NotificationsSettingsActivity(TLObject response, TLRPC$TL_error error) {
    ...
    static final /* synthetic */ void m1235xafa3bc98(TLObject response, TLRPC$TL_error error) {
    }

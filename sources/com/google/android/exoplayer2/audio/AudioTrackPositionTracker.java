...
final class AudioTrackPositionTracker {
    ...
    public void setAudioTrack(AudioTrack audioTrack, int outputEncoding, int outputPcmFrameSize, int bufferSize) {
        ...
        this.bufferSizeUs = this.isOutputPcm ? framesToDurationUs((long) (bufferSize / outputPcmFrameSize)) : C0246C.TIME_UNSET;
        this.lastRawPlaybackHeadPosition = 0;
        this.rawPlaybackHeadWrapCount = 0;
        this.passthroughWorkaroundPauseOffset = 0;
        this.hasData = false;
        this.stopTimestampUs = C0246C.TIME_UNSET;
        this.forceResetWorkaroundTimeMs = C0246C.TIME_UNSET;
        this.latencyUs = 0;
    }
    ...
    public boolean mayHandleBuffer(long writtenFrames) {
        ...
        if (!(!hadData || this.hasData || playState == 1 || this.listener == null)) {
            this.listener.onUnderrun(this.bufferSize, C0246C.usToMs(this.bufferSizeUs));
        }
        return true;
    }
    ...
    public boolean isStalled(long writtenFrames) {
        return this.forceResetWorkaroundTimeMs != C0246C.TIME_UNSET && writtenFrames > 0 && SystemClock.elapsedRealtime() - this.forceResetWorkaroundTimeMs >= FORCE_RESET_WORKAROUND_TIMEOUT_MS;
    }
    ...
    public boolean pause() {
        resetSyncParams();
        if (this.stopTimestampUs != C0246C.TIME_UNSET) {
            return false;
        }
        ((AudioTimestampPoller) Assertions.checkNotNull(this.audioTimestampPoller)).reset();
        return true;
    ...
    private long getPlaybackHeadPosition() {
        ...
        if (Util.SDK_INT <= 28) {
            if (rawPlaybackHeadPosition == 0 && this.lastRawPlaybackHeadPosition > 0 && state == 3) {
                if (this.forceResetWorkaroundTimeMs == C0246C.TIME_UNSET) {

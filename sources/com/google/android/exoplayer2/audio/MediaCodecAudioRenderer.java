...
public class MediaCodecAudioRenderer extends MediaCodecRenderer implements MediaClock {
    ...
    protected float getCodecOperatingRate(float operatingRate, Format format, Format[] streamFormats) {
        return format.sampleRate == -1 ? -1.0f : ((float) format.sampleRate) * operatingRate;
    }
    ...
    private int getCodecMaxInputSize(MediaCodecInfo codecInfo, Format format) {
        if (Util.SDK_INT < 24 && "OMX.google.raw.decoder".equals(codecInfo.name)) {
            ...
            if (Util.SDK_INT == 23) {
                ...
                if (packageManager != null && packageManager.hasSystemFeature(DeviceProperties.FEATURE_TV_3)) {

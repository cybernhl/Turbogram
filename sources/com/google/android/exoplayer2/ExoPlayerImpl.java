...
final class ExoPlayerImpl implements ExoPlayer {
    ...
    private static final class PlaybackInfoUpdate {
        ...
        public PlaybackInfoUpdate(PlaybackInfo playbackInfo, PlaybackInfo previousPlaybackInfo, Set<EventListener> listeners, TrackSelector trackSelector, boolean positionDiscontinuity, int positionDiscontinuityReason, int timelineChangeReason, boolean seekProcessed, boolean playWhenReady, boolean playWhenReadyChanged) {
            ...
            boolean z2 = playWhenReadyChanged || previousPlaybackInfo.playbackState != playbackInfo.playbackState;
            this.playbackStateOrPlayWhenReadyChanged = z2;
            if (previousPlaybackInfo.timeline == playbackInfo.timeline && previousPlaybackInfo.manifest == playbackInfo.manifest) {
                z2 = false;
            } else {
                z2 = true;
            }
            this.timelineOrManifestChanged = z2;
            if (previousPlaybackInfo.isLoading != playbackInfo.isLoading) {
                z2 = true;
            } else {
                z2 = false;
            }
            this.isLoadingChanged = z2;
        ...
        public void notifyListeners() {
            ...
            if (this.trackSelectorResultChanged) {
                ...
                for (EventListener listener22 : this.listeners) {
                    listener22.onTracksChanged(this.playbackInfo.trackGroups, this.playbackInfo.trackSelectorResult.selections);
                }
            }
            if (this.isLoadingChanged) {
                for (EventListener listener222 : this.listeners) {
                    listener222.onLoadingChanged(this.playbackInfo.isLoading);
                }
            }
            if (this.playbackStateOrPlayWhenReadyChanged) {
                for (EventListener listener2222 : this.listeners) {
                    listener2222.onPlayerStateChanged(this.playWhenReady, this.playbackInfo.playbackState);
                }
            }
            if (this.seekProcessed) {
                for (EventListener listener22222 : this.listeners) {
    ...
    public void seekToDefaultPosition(int windowIndex) {
        seekTo(windowIndex, C0246C.TIME_UNSET);
    }
    ...
    public void seekTo(int windowIndex, long positionMs) {
        ...
        if (timeline.isEmpty()) {
            long j;
            if (positionMs == C0246C.TIME_UNSET) {
                j = 0;
            } else {
                j = positionMs;
            }
            ...
            long windowPositionUs = positionMs == C0246C.TIME_UNSET ? timeline.getWindow(windowIndex, this.window).getDefaultPositionUs() : C0246C.msToUs(positionMs);
            Pair<Integer, Long> periodIndexAndPosition = timeline.getPeriodPosition(this.window, this.period, windowIndex, windowPositionUs);
            this.maskingWindowPositionMs = C0246C.usToMs(windowPositionUs);
            this.maskingPeriodIndex = ((Integer) periodIndexAndPosition.first).intValue();
        }
        this.internalPlayer.seekTo(timeline, windowIndex, C0246C.msToUs(positionMs));
        Iterator it = this.listeners.iterator();
        while (it.hasNext()) {
    ...
    public long getDuration() {
        ...
        return C0246C.usToMs(this.period.getAdDurationUs(periodId.adGroupIndex, periodId.adIndexInAdGroup));
    }
    ...
    public long getCurrentPosition() {
        ...
        if (this.playbackInfo.periodId.isAd()) {
            return C0246C.usToMs(this.playbackInfo.positionUs);
        }
    ...
    public long getBufferedPosition() {
        ...
        if (this.playbackInfo.loadingMediaPeriodId.equals(this.playbackInfo.periodId)) {
            return C0246C.usToMs(this.playbackInfo.bufferedPositionUs);
        }
        return getDuration();
    }
    ...
    public int getBufferedPercentage() {
        ...
        if (position == C0246C.TIME_UNSET || duration == C0246C.TIME_UNSET) {
            return 0;
        }
        if (duration != 0) {
    ...
    public long getTotalBufferedDuration() {
        return Math.max(0, C0246C.usToMs(this.playbackInfo.totalBufferedDurationUs));
    }
    ...
    public long getContentPosition() {
        ...
        return this.period.getPositionInWindowMs() + C0246C.usToMs(this.playbackInfo.contentPositionUs);
    }
    ...
    public long getContentBufferedPosition() {
        ...
        if (this.playbackInfo.loadingMediaPeriodId.isAd()) {
            Period loadingPeriod = this.playbackInfo.timeline.getPeriod(this.playbackInfo.loadingMediaPeriodId.periodIndex, this.period);
    ...
    private void handlePlaybackInfo(PlaybackInfo playbackInfo, int operationAcks, boolean positionDiscontinuity, int positionDiscontinuityReason) {
        ...
        if (this.pendingOperationAcks == 0) {
            if (playbackInfo.startPositionUs == C0246C.TIME_UNSET) {
    ...
    private PlaybackInfo getResetPlaybackInfo(boolean resetPosition, boolean resetState, int playbackState) {
        ...
        return new PlaybackInfo(resetState ? Timeline.EMPTY : this.playbackInfo.timeline, resetState ? null : this.playbackInfo.manifest, this.playbackInfo.periodId, this.playbackInfo.startPositionUs, this.playbackInfo.contentPositionUs, playbackState, false, resetState ? TrackGroupArray.EMPTY : this.playbackInfo.trackGroups, resetState ? this.emptyTrackSelectorResult : this.playbackInfo.trackSelectorResult, this.playbackInfo.periodId, this.playbackInfo.startPositionUs, 0, this.playbackInfo.startPositionUs);
    }
    ...
    private long periodPositionUsToWindowPositionMs(MediaPeriodId periodId, long positionUs) {
        long positionMs = C0246C.usToMs(positionUs);
        this.playbackInfo.timeline.getPeriod(periodId.periodIndex, this.period);

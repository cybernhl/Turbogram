...
final class MediaPeriodQueue {
    ...
    public boolean shouldLoadNextMediaPeriod() {
        return this.loading == null || (!this.loading.info.isFinal && this.loading.isFullyBuffered() && this.loading.info.durationUs != C0246C.TIME_UNSET && this.length < MAXIMUM_BUFFER_AHEAD_PERIODS);
    }
    ...
    private MediaPeriodInfo getFollowingMediaPeriodInfo(MediaPeriodHolder mediaPeriodHolder, long rendererPositionUs) {
        ...
        if (mediaPeriodInfo.isLastInTimelinePeriod) {
            ...
            if (this.timeline.getWindow(nextWindowIndex, this.window).firstPeriodIndex == nextPeriodIndex) {
                Pair<Integer, Long> defaultPosition = this.timeline.getPeriodPosition(this.window, this.period, nextWindowIndex, C0246C.TIME_UNSET, Math.max(0, (mediaPeriodHolder.getRendererOffset() + mediaPeriodInfo.durationUs) - rendererPositionUs));
        ...
        if (currentPeriodId.isAd()) {
            ...
            if (nextAdGroupIndex == -1) {
                return getMediaPeriodInfoForContent(currentPeriodId.periodIndex, mediaPeriodInfo.id.endPositionUs, currentPeriodId.windowSequenceNumber);
            }
            ...
            if (this.period.isAdAvailable(nextAdGroupIndex, adIndexInAdGroup)) {
                return getMediaPeriodInfoForAd(currentPeriodId.periodIndex, nextAdGroupIndex, adIndexInAdGroup, mediaPeriodInfo.id.endPositionUs, currentPeriodId.windowSequenceNumber);
            }
            return null;
        } else {
    ...
    private MediaPeriodInfo getUpdatedMediaPeriodInfo(MediaPeriodInfo info, MediaPeriodId newId) {
        ...
        long durationUs = newId.isAd() ? this.period.getAdDurationUs(newId.adGroupIndex, newId.adIndexInAdGroup) : newId.endPositionUs == Long.MIN_VALUE ? this.period.getDurationUs() : newId.endPositionUs;
        return new MediaPeriodInfo(newId, startPositionUs, info.contentPositionUs, durationUs, isLastInPeriod, isLastInTimeline);
    }
    ...
    private MediaPeriodInfo getMediaPeriodInfoForAd(int periodIndex, int adGroupIndex, int adIndexInAdGroup, long contentPositionUs, long windowSequenceNumber) {
        ...
        return new MediaPeriodInfo(id, adIndexInAdGroup == this.period.getFirstAdIndexToPlay(adGroupIndex) ? this.period.getAdResumePositionUs() : 0, contentPositionUs, this.timeline.getPeriod(id.periodIndex, this.period).getAdDurationUs(id.adGroupIndex, id.adIndexInAdGroup), isLastInPeriod, isLastInTimeline);
    }
    ...
    private MediaPeriodInfo getMediaPeriodInfoForContent(int periodIndex, long startPositionUs, long windowSequenceNumber) {
        ...
        return new MediaPeriodInfo(id, startPositionUs, C0246C.TIME_UNSET, durationUs, isLastInPeriod, isLastInTimeline);
    }
    ...
    private boolean isLastInPeriod(MediaPeriodId id) {
        ...
        if (this.period.getAdGroupTimeUs(lastAdGroupIndex) == Long.MIN_VALUE) {
            ...
            return z;
        } else if (isAd || id.endPositionUs != Long.MIN_VALUE) {
            return false;
        } else {
            return true;
        }

...
final class ExoPlayerImplInternal implements Callback, PlaybackParameterListener, Sender, MediaPeriod.Callback, SourceInfoRefreshListener, InvalidationListener {
    ...
    public ExoPlayerImplInternal(Renderer[] renderers, TrackSelector trackSelector, TrackSelectorResult emptyTrackSelectorResult, LoadControl loadControl, BandwidthMeter bandwidthMeter, boolean playWhenReady, int repeatMode, boolean shuffleModeEnabled, Handler eventHandler, ExoPlayer player, Clock clock) {
        ...
        this.playbackInfo = PlaybackInfo.createDummy(C0246C.TIME_UNSET, emptyTrackSelectorResult);
    ...
    private void updatePlaybackPositions() throws ExoPlaybackException {
        if (this.queue.hasPlayingPeriod()) {
            ...
            if (periodPositionUs != C0246C.TIME_UNSET) {
    ...
    private void seekToInternal(SeekPosition seekPosition) throws ExoPlaybackException {
        ...
        if (resolvedSeekPosition == null) {
            ...
            periodPositionUs = C0246C.TIME_UNSET;
            contentPositionUs = C0246C.TIME_UNSET;
            ...
            if (periodId.isAd()) {
                ...
                seekPositionAdjusted = seekPosition.windowPositionUs == C0246C.TIME_UNSET;
            }
        }
        try {
            if (this.mediaSource == null || this.pendingPrepareCount > 0) {
                this.pendingInitialSeekPosition = seekPosition;
            } else if (periodPositionUs == C0246C.TIME_UNSET) {
                setState(4);
                ...
                if (periodId.equals(this.playbackInfo.periodId)) {
                    ...
                    if (C0246C.usToMs(newPeriodPositionUs) == C0246C.usToMs(this.playbackInfo.positionUs)) {
    ...
    private void resetInternal(boolean releaseMediaSource, boolean resetPosition, boolean resetState) {
        ...
        long startPositionUs = resetPosition ? C0246C.TIME_UNSET : this.playbackInfo.positionUs;
        long contentPositionUs = resetPosition ? C0246C.TIME_UNSET : this.playbackInfo.contentPositionUs;
        if (resetState) {
    ...
    private void sendMessageInternal(PlayerMessage message) throws ExoPlaybackException {
        if (message.getPositionMs() == C0246C.TIME_UNSET) {
    ...
    private boolean resolvePendingMessagePosition(PendingMessageInfo pendingMessageInfo) {
        if (pendingMessageInfo.resolvedPeriodUid == null) {
            Pair<Integer, Long> periodPosition = resolveSeekPosition(new SeekPosition(pendingMessageInfo.message.getTimeline(), pendingMessageInfo.message.getWindowIndex(), C0246C.msToUs(pendingMessageInfo.message.getPositionMs())), false);
    ...
    private boolean isTimelineReady() {
        ...
        return playingPeriodDurationUs == C0246C.TIME_UNSET || this.playbackInfo.positionUs < playingPeriodDurationUs || (playingPeriodHolder.next != null && (playingPeriodHolder.next.prepared || playingPeriodHolder.next.info.id.isAd()));
    }
            ...
            if (this.pendingPrepareCount > 0) {
                ...
                if (this.pendingInitialSeekPosition != null) {
                    try {
                        ...
                        this.playbackInfo = this.playbackInfo.fromNewPosition(getFirstMediaPeriodId(), C0246C.TIME_UNSET, C0246C.TIME_UNSET);
                        throw e;
                    }
                } else if (this.playbackInfo.startPositionUs != C0246C.TIME_UNSET) {
                } else {
                    if (timeline.isEmpty()) {
                        handleSourceInfoRefreshEndedPlayback();
                        return;
                    }
                    defaultPosition = getPeriodPosition(timeline, timeline.getFirstWindowIndex(this.shuffleModeEnabled), C0246C.TIME_UNSET);
                ...
                if (periodIndex == -1) {
                    ...
                    defaultPosition = getPeriodPosition(timeline2, timeline.getPeriod(newPeriodIndex, this.period).windowIndex, C0246C.TIME_UNSET);
    ...
    private Pair<Integer, Long> resolveSeekPosition(SeekPosition seekPosition, boolean trySubsequentPeriods) {
        ...
        try {
            ...
            if (trySubsequentPeriods) {
                ...
                if (periodIndex != -1) {
                    return getPeriodPosition(timeline, timeline.getPeriod(periodIndex, this.period).windowIndex, C0246C.TIME_UNSET);
                }
            }
            return null;
        ...
        if (this.mediaSource != null) {
            ...
            if (this.queue.hasPlayingPeriod()) {
                ...
                if (readingPeriodHolder.info.isFinal) {
                    ...
                    boolean initialDiscontinuity = readingPeriodHolder.mediaPeriod.readDiscontinuity() != C0246C.TIME_UNSET;
                    for (i = 0; i < this.renderers.length; i++) {
                        renderer = this.renderers[i];

...
public abstract class MediaCodecRenderer extends BaseRenderer {
        ...
        if (this.codec == null && this.format != null) {
            ...
            try {
                if (initCodecWithFallback(wrappedMediaCrypto, drmSessionRequiresSecureDecoder)) {
                    ...
                    this.codecHotswapDeadlineMs = getState() == 2 ? SystemClock.elapsedRealtime() + 1000 : C0246C.TIME_UNSET;
                    resetInputBuffer();
                    resetOutputBuffer();
    ...
    protected void releaseCodec() {
        this.codecHotswapDeadlineMs = C0246C.TIME_UNSET;
        resetInputBuffer();
        resetOutputBuffer();
    ...
    private void initCodec(MediaCodecInfo codecInfo, MediaCrypto crypto) throws Exception {
        ...
        try {
            ...
            configureCodec(codecInfo, codec, this.format, crypto, configureWithOperatingRate ? this.codecOperatingRate : -1.0f);
    ...
    public boolean isReady() {
        if (this.format == null || this.waitingForKeys || (!isSourceReady() && !hasOutputBuffer() && (this.codecHotswapDeadlineMs == C0246C.TIME_UNSET || SystemClock.elapsedRealtime() >= this.codecHotswapDeadlineMs))) {
            return false;
        }
        return true;
    }
            ...
            if (this.codecOperatingRate != codecOperatingRate) {
                ...
                if (this.codec != null && this.codecReinitializationState == 0) {
                    if (codecOperatingRate == -1.0f && this.codecConfiguredWithOperatingRate) {
                        reinitializeCodec();
                    } else if (codecOperatingRate == -1.0f) {
                    } else {
    ...
    private void processOutputFormat() throws ExoPlaybackException {
        ...
        if (this.codecAdaptationWorkaroundMode != 0 && format.getInteger("width") == 32 && format.getInteger(MonthView.VIEW_PARAMS_HEIGHT) == 32) {

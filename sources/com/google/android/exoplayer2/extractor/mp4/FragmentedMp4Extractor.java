...
public final class FragmentedMp4Extractor implements Extractor {
    ...
    private static final class TrackBundle {
        ...
        public void seek(long timeUs) {
            long timeMs = C0246C.usToMs(timeUs);
        ...
        private TrackEncryptionBox getEncryptionBox() {
            ...
            return this.track.getSampleDescriptionEncryptionBox(sampleDescriptionIndex);
        }
    }
    ...
    public FragmentedMp4Extractor(int flags, @Nullable TimestampAdjuster timestampAdjuster, @Nullable Track sideloadedTrack, @Nullable DrmInitData sideloadedDrmInitData, List<Format> closedCaptionFormats, @Nullable TrackOutput additionalEmsgTrackOutput) {
        ...
        this.pendingSeekTimeUs = C0246C.TIME_UNSET;
        this.segmentIndexEarliestPresentationTimeUs = C0246C.TIME_UNSET;
    ...
    private void onMoovContainerAtomRead(ContainerAtom moov) throws ParserException {
        ...
        long duration = C0246C.TIME_UNSET;
        ...
        for (i = 0; i < trackCount; i++) {
            track = (Track) tracks.valueAt(i);
            ((TrackBundle) this.trackBundles.get(track.id)).init(track, getDefaultSampleValues(defaultSampleValuesArray, track.id));
        }
    }
    ...
    private void onMoofContainerAtomRead(ContainerAtom moof) throws ParserException {
        ...
        if (this.pendingSeekTimeUs != C0246C.TIME_UNSET) {
            trackCount = this.trackBundles.size();
            for (i = 0; i < trackCount; i++) {
    ...
    private void maybeInitExtraTracks() {
        ...
        if (this.cea608TrackOutputs == null) {
            ...
            for (int i2 = 0; i2 < this.cea608TrackOutputs.length; i2++) {
                TrackOutput output = this.extractorOutput.track((this.trackBundles.size() + 1) + i2, 3);
                output.format((Format) this.closedCaptionFormats.get(i2));
                this.cea608TrackOutputs[i2] = output;
            }
        }
    }
    ...
    private void onEmsgLeafAtomRead(ParsableByteArray atom) {
        if (this.emsgTrackOutputs != null && this.emsgTrackOutputs.length != 0) {
            ...
            if (this.segmentIndexEarliestPresentationTimeUs != C0246C.TIME_UNSET) {
    ...
    private static void parseTraf(ContainerAtom traf, SparseArray<TrackBundle> trackBundleArray, int flags, byte[] extendedTypeScratch) throws ParserException {
        TrackBundle trackBundle = parseTfhd(traf.getLeafAtomOfType(Atom.TYPE_tfhd).data, trackBundleArray);
        if (trackBundle != null) {
        ...
        if (track.nalUnitLengthFieldLength != 0) {
            ...
            while (this.sampleBytesWritten < this.sampleSize) {
                if (this.sampleCurrentNalBytesRemaining == 0) {
                    ...
                    boolean z = this.cea608TrackOutputs.length > 0 && NalUnitUtil.isNalUnitSei(track.format.sampleMimeType, nalPrefixData[4]);
    ...
    private void outputPendingMetadataSamples(long sampleTimeUs) {
        while (!this.pendingMetadataSampleInfos.isEmpty()) {
            ...
            for (TrackOutput emsgTrackOutput : this.emsgTrackOutputs) {
                emsgTrackOutput.sampleMetadata(sampleInfo.presentationTimeDeltaUs + sampleTimeUs, 1, sampleInfo.size, this.pendingMetadataSampleBytes, null);
            }
        }
    }

...
final class AtomParsers {
    ...
    public static TrackSampleTable parseStbl(Track track, ContainerAtom stblAtom, GaplessInfoHolder gaplessInfoHolder) throws ParserException {
        ...
        if (sampleCount == 0) {
            return new TrackSampleTable(track, new long[0], new int[0], 0, new long[0], new int[0], C0246C.TIME_UNSET);
        }
        Object offsets;
        Object sizes;
        long[] timestamps;
        ...
        if (track.editListDurations.length == 1 && track.editListDurations[0] == 0) {
            ...
            return new TrackSampleTable(track, offsets, sizes, maximumSize, timestamps, flags, Util.scaleLargeTimestamp(duration - editStartTime, 1000000, track.timescale));
        }
        ...
        for (i = 0; i < track.editListDurations.length; i++) {
            ...
            if (editMediaTime != -1) {
                ...
                if (startIndex >= endIndex || (editedFlags[sampleIndex] & 1) != 0) {
                    int j = startIndex;
                    int editedMaximumSize2 = editedMaximumSize;
                    while (j < endIndex) {
                        ...
                        if (!copyMetadata || editedSizes[sampleIndex] <= editedMaximumSize2) {
        ...
        return new TrackSampleTable(track, editedOffsets, editedSizes, editedMaximumSize, editedTimestamps, editedFlags, Util.scaleLargeTimestamp(pts, 1000000, track.timescale));
    }
    ...
    static Pair<Integer, TrackEncryptionBox> parseCommonEncryptionSinfFromParent(ParsableByteArray parent, int position, int size) {
        ...
        if (!C0246C.CENC_TYPE_cenc.equals(schemeType) && !C0246C.CENC_TYPE_cbc1.equals(schemeType) && !C0246C.CENC_TYPE_cens.equals(schemeType) && !C0246C.CENC_TYPE_cbcs.equals(schemeType)) {
            return null;
        }
        boolean z2;

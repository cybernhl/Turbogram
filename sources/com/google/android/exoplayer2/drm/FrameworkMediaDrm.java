...
public final class FrameworkMediaDrm implements ExoMediaDrm<FrameworkMediaCrypto> {
    ...
    private FrameworkMediaDrm(UUID uuid) throws UnsupportedSchemeException {
        ...
        if (Util.SDK_INT < 27 && C0246C.CLEARKEY_UUID.equals(uuid)) {
            uuid = C0246C.COMMON_PSSH_UUID;
        }
        this.uuid = uuid;
        this.mediaDrm = new MediaDrm(uuid);
        if (C0246C.WIDEVINE_UUID.equals(uuid) && needsForceL3Workaround()) {
    ...
    public void setOnKeyStatusChangeListener(final OnKeyStatusChangeListener<? super FrameworkMediaCrypto> listener) {
        ...
        if (listener == null) {
            ...
            onKeyStatusChangeListener = new MediaDrm.OnKeyStatusChangeListener() {
                public void onKeyStatusChange(@NonNull MediaDrm md, @NonNull byte[] sessionId, @NonNull List<KeyStatus> keyInfo, boolean hasNewUsableKey) {
    ...
    public KeyRequest getKeyRequest(byte[] scope, byte[] init, String mimeType, int keyType, HashMap<String, String> optionalParameters) throws NotProvisionedException {
        if ((Util.SDK_INT < 21 && C0246C.WIDEVINE_UUID.equals(this.uuid)) || (C0246C.PLAYREADY_UUID.equals(this.uuid) && "Amazon".equals(Util.MANUFACTURER) && ("AFTB".equals(Util.MODEL) || "AFTS".equals(Util.MODEL) || "AFTM".equals(Util.MODEL)))) {
            byte[] psshData = PsshAtomUtil.parseSchemeSpecificData(init, this.uuid);
            if (psshData != null) {
                init = psshData;
            }
        }
        if (Util.SDK_INT < 26 && C0246C.CLEARKEY_UUID.equals(this.uuid) && (MimeTypes.VIDEO_MP4.equals(mimeType) || MimeTypes.AUDIO_MP4.equals(mimeType))) {
            mimeType = "cenc";
        }
        ...
        if (C0246C.CLEARKEY_UUID.equals(this.uuid)) {
    ...
    public FrameworkMediaCrypto createMediaCrypto(byte[] initData) throws MediaCryptoException {
        boolean forceAllowInsecureDecoderComponents = Util.SDK_INT < 21 && C0246C.WIDEVINE_UUID.equals(this.uuid) && "L3".equals(getPropertyString("securityLevel"));

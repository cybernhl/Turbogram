...
class DefaultDrmSession<T extends ExoMediaCrypto> implements DrmSession<T> {
    ...
    public DefaultDrmSession(UUID uuid, ExoMediaDrm<T> mediaDrm, ProvisioningManager<T> provisioningManager, @Nullable SchemeData schemeData, int mode, @Nullable byte[] offlineLicenseKeySetId, HashMap<String, String> optionalKeyRequestParameters, MediaDrmCallback callback, Looper playbackLooper, EventDispatcher<DefaultDrmSessionEventListener> eventDispatcher, int initialDrmRequestRetryCount) {
        this.uuid = uuid;
    ...
    private long getLicenseDurationRemainingSec() {
        if (!C0246C.WIDEVINE_UUID.equals(this.uuid)) {

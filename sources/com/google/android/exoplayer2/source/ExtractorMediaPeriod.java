...
final class ExtractorMediaPeriod implements ExtractorOutput, MediaPeriod, UpstreamFormatChangedListener, Callback<ExtractingLoadable>, ReleaseCallback {
    ...
    public ExtractorMediaPeriod(Uri uri, DataSource dataSource, Extractor[] extractors, int minLoadableRetryCount, EventDispatcher eventDispatcher, Listener listener, Allocator allocator, @Nullable String customCacheKey, int continueLoadingCheckIntervalBytes) {
        this.uri = uri;
        ...
        this.maybeFinishPrepareRunnable = new C03181();
        this.onContinueLoadingRequestedRunnable = new C03192();
        this.handler = new Handler();
        this.sampleQueueTrackIds = new int[0];
        this.sampleQueues = new SampleQueue[0];
        this.pendingResetPositionUs = C0246C.TIME_UNSET;
        this.length = -1;
        this.durationUs = C0246C.TIME_UNSET;
    ...
    public void onLoadCompleted(ExtractingLoadable loadable, long elapsedRealtimeMs, long loadDurationMs) {
        if (this.durationUs == C0246C.TIME_UNSET) {
        ...
        this.eventDispatcher.loadCompleted(loadable.dataSpec, loadable.dataSource.getLastOpenedUri(), 1, -1, null, 0, null, loadable.seekTimeUs, this.durationUs, elapsedRealtimeMs, loadDurationMs, loadable.dataSource.getBytesRead());
    ...
    public void onLoadCanceled(ExtractingLoadable loadable, long elapsedRealtimeMs, long loadDurationMs, boolean released) {
        this.eventDispatcher.loadCanceled(loadable.dataSpec, loadable.dataSource.getLastOpenedUri(), 1, -1, null, 0, null, loadable.seekTimeUs, this.durationUs, elapsedRealtimeMs, loadDurationMs, loadable.dataSource.getBytesRead());
        if (!released) {
            copyLengthFromLoader(loadable);
            for (SampleQueue sampleQueue : this.sampleQueues) {
                sampleQueue.reset();
    ...
    public LoadErrorAction onLoadError(ExtractingLoadable loadable, long elapsedRealtimeMs, long loadDurationMs, IOException error, int errorCount) {
        ...
        if (isLoadableExceptionFatal(error)) {
            ...
            retryAction = configureRetry(loadable, extractedSamplesCount) ? extractedSamplesCount > this.extractedSamplesCountAtStartOfLoad ? Loader.RETRY_RESET_ERROR_COUNT : Loader.RETRY : Loader.DONT_RETRY;
        }
        boolean wasCanceled = retryAction == Loader.DONT_RETRY || retryAction == Loader.DONT_RETRY_FATAL;
        this.eventDispatcher.loadError(loadable.dataSpec, loadable.dataSource.getLastOpenedUri(), 1, -1, null, 0, null, loadable.seekTimeUs, this.durationUs, elapsedRealtimeMs, loadDurationMs, loadable.dataSource.getBytesRead(), error, wasCanceled);
        return retryAction;
    }
    ...
    private void maybeFinishPrepare() {
        if (!this.released && !this.prepared && this.seekMap != null && this.sampleQueuesBuilt) {
            ...
            if (this.minLoadableRetryCount == -1 && this.length == -1 && this.seekMap.getDurationUs() == C0246C.TIME_UNSET) {
    ...
    private void startLoading() {
        ...
        if (this.prepared) {
            ...
            if (this.durationUs == C0246C.TIME_UNSET || this.pendingResetPositionUs < this.durationUs) {
        ...
        this.eventDispatcher.loadStarted(loadable.dataSpec, loadable.dataSpec.uri, 1, -1, null, 0, null, loadable.seekTimeUs, this.durationUs, this.loader.startLoading(loadable, this, this.actualMinLoadableRetryCount));
    }

    private boolean configureRetry(ExtractingLoadable loadable, int currentExtractedSampleCount) {
        int i = 0;
        if (this.length != -1 || (this.seekMap != null && this.seekMap.getDurationUs() != C0246C.TIME_UNSET)) {
    ...
    private boolean isPendingReset() {
        return this.pendingResetPositionUs != C0246C.TIME_UNSET;
    }

...
public final class HlsMediaSource extends BaseMediaSource implements PrimaryPlaylistListener {
    ...
    private HlsMediaSource(Uri manifestUri, HlsDataSourceFactory dataSourceFactory, HlsExtractorFactory extractorFactory, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory, LoadErrorHandlingPolicy<Chunk> chunkLoadErrorHandlingPolicy, int minLoadableRetryCount, HlsPlaylistTracker playlistTracker, boolean allowChunklessPreparation, @Nullable Object tag) {
    ...
    public void onPrimaryPlaylistRefreshed(HlsMediaPlaylist playlist) {
        ...
        long windowStartTimeMs = playlist.hasProgramDateTime ? C0246C.usToMs(playlist.startTimeUs) : C0246C.TIME_UNSET;
        long presentationStartTimeMs = (playlist.playlistType == 2 || playlist.playlistType == 1) ? windowStartTimeMs : C0246C.TIME_UNSET;
        long windowDefaultStartPositionUs = playlist.startOffsetUs;
        if (this.playlistTracker.isLive()) {
            boolean z;
            long offsetFromInitialStartTimeUs = playlist.startTimeUs - this.playlistTracker.getInitialStartTimeUs();
            long periodDurationUs = playlist.hasEndTag ? offsetFromInitialStartTimeUs + playlist.durationUs : C0246C.TIME_UNSET;
            List<Segment> segments = playlist.segments;
            if (windowDefaultStartPositionUs == C0246C.TIME_UNSET) {
                if (segments.isEmpty()) {

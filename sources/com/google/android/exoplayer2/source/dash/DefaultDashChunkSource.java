...
public class DefaultDashChunkSource implements DashChunkSource {
    ...
    private long liveEdgeTimeUs = C0246C.TIME_UNSET;
    ...
    public void getNextChunk(long playbackPositionUs, long loadPositionUs, List<? extends MediaChunk> queue, ChunkHolder out) {
        if (this.fatalError == null) {
            ...
            long presentationPositionUs = (C0246C.msToUs(this.manifest.availabilityStartTimeMs) + C0246C.msToUs(this.manifest.getPeriod(this.periodIndex).startMs)) + loadPositionUs;
            if (this.playerTrackEmsgHandler == null || !this.playerTrackEmsgHandler.maybeRefreshManifestBeforeLoadingNextChunk(presentationPositionUs)) {
                ...
                if (availableSegmentCount == -1) {
                    long liveEdgeTimeInPeriodUs = (getNowUnixTimeUs() - C0246C.msToUs(this.manifest.availabilityStartTimeMs)) - C0246C.msToUs(this.manifest.getPeriod(this.periodIndex).startMs);
                    if (this.manifest.timeShiftBufferDepthMs != C0246C.TIME_UNSET) {
                        firstAvailableSegmentNum = Math.max(firstAvailableSegmentNum, representationHolder.getSegmentNum(liveEdgeTimeInPeriodUs - C0246C.msToUs(this.manifest.timeShiftBufferDepthMs)));
                    }
                ...
                out.chunk = newMediaChunk(representationHolder, this.dataSource, this.trackType, this.trackSelection.getSelectedFormat(), this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), segmentNum, (int) Math.min((long) this.maxSegmentsPerLoad, (lastAvailableSegmentNum - segmentNum) + 1), queue.isEmpty() ? loadPositionUs : C0246C.TIME_UNSET);
            }
        }
    }
    ...
    private void updateLiveEdgeTimeUs(RepresentationHolder representationHolder, long lastAvailableSegmentNum) {
        this.liveEdgeTimeUs = this.manifest.dynamic ? representationHolder.getSegmentEndTimeUs(lastAvailableSegmentNum) : C0246C.TIME_UNSET;
    }
    ...
    private long resolveTimeToLiveEdgeUs(long playbackPositionUs) {
        boolean resolveTimeToLiveEdgePossible = this.manifest.dynamic && this.liveEdgeTimeUs != C0246C.TIME_UNSET;

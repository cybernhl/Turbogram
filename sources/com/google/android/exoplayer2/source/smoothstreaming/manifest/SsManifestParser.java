...
public class SsManifestParser implements Parser<SsManifest> {
    ...
    private static class QualityLevelParser extends ElementParser {
        ...
        public void parseStartTag(XmlPullParser parser) throws ParserException {
            ...
            if (type == 2) {
                this.format = Format.createVideoContainerFormat(id, name, MimeTypes.VIDEO_MP4, sampleMimeType, null, bitrate, parseRequiredInt(parser, KEY_MAX_WIDTH), parseRequiredInt(parser, KEY_MAX_HEIGHT), -1.0f, buildCodecSpecificData(parser.getAttributeValue(null, KEY_CODEC_PRIVATE_DATA)), 0);
            } else if (type == 1) {
                ...
                String str = id;
                String str2 = name;
                this.format = Format.createAudioContainerFormat(str, str2, MimeTypes.AUDIO_MP4, sampleMimeType, null, bitrate, channels, samplingRate, codecSpecificData, 0, (String) getNormalizedAttribute(KEY_LANGUAGE));
            } else if (type == 3) {
                String str3 = id;
                String str4 = name;
                this.format = Format.createTextContainerFormat(str3, str4, MimeTypes.APPLICATION_MP4, sampleMimeType, null, bitrate, 0, (String) getNormalizedAttribute(KEY_LANGUAGE));
            } else {
                this.format = Format.createContainerFormat(id, name, MimeTypes.APPLICATION_MP4, sampleMimeType, null, bitrate, 0, null);
            }
        }
    ...
    private static class StreamIndexParser extends ElementParser {
        ...
        private void parseStreamFragmentStartTag(XmlPullParser parser) throws ParserException {
            ...
            long startTime = parseLong(parser, KEY_FRAGMENT_START_TIME, C0246C.TIME_UNSET);
            if (startTime == C0246C.TIME_UNSET) {
                if (chunkIndex == 0) {
                    startTime = 0;
            ...
            this.lastChunkDuration = parseLong(parser, KEY_FRAGMENT_DURATION, C0246C.TIME_UNSET);
            long repeatCount = parseLong(parser, KEY_FRAGMENT_REPEAT_COUNT, 1);
            if (repeatCount <= 1 || this.lastChunkDuration != C0246C.TIME_UNSET) {

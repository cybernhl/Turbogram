...
public final class SsMediaSource extends BaseMediaSource implements Callback<ParsingLoadable<SsManifest>> {
    ...
    private SsMediaSource(SsManifest manifest, Uri manifestUri, com.google.android.exoplayer2.upstream.DataSource.Factory manifestDataSourceFactory, Parser<? extends SsManifest> manifestParser, com.google.android.exoplayer2.source.smoothstreaming.SsChunkSource.Factory chunkSourceFactory, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory, int minLoadableRetryCount, long livePresentationDelayMs, @Nullable Object tag) {
        boolean z = true;
    ...
    private void processManifest() {
        ...
        if (startTimeUs2 == Long.MAX_VALUE) {
            timeline = new SinglePeriodTimeline(this.manifest.isLive ? C0246C.TIME_UNSET : 0, 0, 0, 0, true, this.manifest.isLive, this.tag);
            startTimeUs = startTimeUs2;
        } else if (this.manifest.isLive) {
            if (this.manifest.dvrWindowLengthUs == C0246C.TIME_UNSET || this.manifest.dvrWindowLengthUs <= 0) {
                startTimeUs = startTimeUs2;
            } else {
                startTimeUs = Math.max(startTimeUs2, endTimeUs - this.manifest.dvrWindowLengthUs);
            }
            durationUs = endTimeUs - startTimeUs;
            long defaultStartPositionUs = durationUs - C0246C.msToUs(this.livePresentationDelayMs);
            ...
            Timeline singlePeriodTimeline = new SinglePeriodTimeline(C0246C.TIME_UNSET, durationUs, startTimeUs, defaultStartPositionUs, true, true, this.tag);
        } else {
            durationUs = this.manifest.durationUs != C0246C.TIME_UNSET ? this.manifest.durationUs : endTimeUs - startTimeUs2;
    ...
    private void scheduleManifestRefresh() {
        if (this.manifest.isLive) {
            this.manifestRefreshHandler.postDelayed(new C03391(), Math.max(0, (this.manifestLoadStartTimestamp + DefaultRenderersFactory.DEFAULT_ALLOWED_VIDEO_JOINING_TIME_MS) - SystemClock.elapsedRealtime()));
        }
    }

...
public class VersionHelper {
    ...
    class C06481 implements Comparator<JSONObject> {
        C06481() {
        }

        public int compare(JSONObject object1, JSONObject object2) {
            try {
                return object1.getInt(ProviderConstants.API_COLNAME_FEATURE_VERSION) > object2.getInt(ProviderConstants.API_COLNAME_FEATURE_VERSION) ? 0 : 0;
    ...
    private void loadVersions(String infoJSON) {
        ...
        try {
            ...
            for (int index = 0; index < versions.length(); index++) {
                ...
                if (entry.getInt(ProviderConstants.API_COLNAME_FEATURE_VERSION) > versionCode) {
                    largerVersionCode = true;
                } else {
                    largerVersionCode = false;
                }
                boolean newerApkFile;
                if (entry.getInt(ProviderConstants.API_COLNAME_FEATURE_VERSION) == versionCode && isNewerThanLastUpdateTime(this.mContext, entry.getLong(Param.TIMESTAMP))) {
                    newerApkFile = true;
                } else {
                    newerApkFile = false;
                }
                if (largerVersionCode || newerApkFile) {
                    this.mNewest = entry;
                    versionCode = entry.getInt(ProviderConstants.API_COLNAME_FEATURE_VERSION);
                }
    ...
    private void sortVersions() {
        Collections.sort(this.mSortedVersions, new C06481());
    }

    public String getVersionString() {
        return failSafeGetStringFromJSON(this.mNewest, "shortversion", "") + " (" + failSafeGetStringFromJSON(this.mNewest, ProviderConstants.API_COLNAME_FEATURE_VERSION, "") + ")";
    }
    ...
    private String getRestoreButton(JSONObject version) {
        ...
        if (!TextUtils.isEmpty(getVersionID(version))) {
            result.append(String.format("<a href='restore:%s' style='%s'>%s</a>", new Object[]{getVersionID(version), "background: #c8c8c8; color: #000; display: block; float: right; padding: 7px; margin: 0px 10px 10px; text-decoration: none;", this.mContext.getString(C0625R.string.hockeyapp_update_restore)}));
        }
        return result.toString();
    }
    ...
    private String getVersionLine(int count, JSONObject version) {
        ...
        if (count == 0) {
            result.append(this.mContext.getString(C0625R.string.hockeyapp_update_newest_version)).append(':');
        } else {
            String versionString = String.format(this.mContext.getString(C0625R.string.hockeyapp_update_version), new Object[]{versionName});
            ...
            if (versionCode != newestCode && versionCode == this.mCurrentVersionCode) {
                ...
                result.append(String.format("[%s]", new Object[]{this.mContext.getString(C0625R.string.hockeyapp_update_already_installed)}));
            }
        }
    ...
    private int getVersionCode(JSONObject version) {
        int versionCode = 0;
        try {
            versionCode = version.getInt(ProviderConstants.API_COLNAME_FEATURE_VERSION);
        } catch (JSONException e) {
        }
        return versionCode;
    }
    ...
    private String getVersionNotes(JSONObject version) {
        ...
        if (notes.trim().length() == 0) {
            result.append(String.format("<em>%s</em>", new Object[]{this.mContext.getString(C0625R.string.hockeyapp_update_no_info)}));
        } else {
            result.append(notes);
        }
        result.append("</div>");

...
public class CheckUpdateTaskWithUI extends CheckUpdateTask {
    ...
    class C06392 implements OnCancelListener {
        C06392() {
        }
    ...
    private void showDialog(final Activity activity, final JSONArray updateInfo) {
        if (activity != null && !activity.isFinishing()) {
            ...
            builder.setTitle(C0625R.string.hockeyapp_update_dialog_title);
            if (this.mandatory.booleanValue()) {
                String appName = Util.getAppName(activity);
                Toast.makeText(activity, activity.getString(C0625R.string.hockeyapp_update_mandatory_toast, new Object[]{appName}), 1).show();
                startUpdateIntent(activity, updateInfo, Boolean.valueOf(true));
                return;
            }
            builder.setMessage(C0625R.string.hockeyapp_update_dialog_message);
            builder.setNegativeButton(C0625R.string.hockeyapp_update_dialog_negative_button, new C06381());
            builder.setOnCancelListener(new C06392());
            builder.setPositiveButton(C0625R.string.hockeyapp_update_dialog_positive_button, new OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
